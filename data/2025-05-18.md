...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型需要数十亿参数和巨额计算资源，研究旨在探索资源效率更高的上下文嵌入子任务解决方案。

Method: 使用GCN编码全局上下文信息，与LSTM局部序列建模结合，在自定义维基百科语料库上进行低资源训练。

Result: 实验表明该混合模型在有限资源条件下能有效预测下一个单词，性能表现良好。

Conclusion: GCN与LSTM的协同架构为资源受限场景下的语言建模提供了可行替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法使用标量奖励信号，无法区分不同语义的推理路径，导致多样性-质量矛盾（优质多样样本被同等对待）。

Method: DRA方法：1. 利用子模互信息(SMI)计算语义多样性；2. 降低冗余样本奖励，增强多样样本奖励；3. 兼容GRPO及其变体DR.GRPO，形成DRA-GRPO/DGA-DR.GRPO混合算法。

Result: 在5个数学推理基准测试中：1. 平均准确率58.2%（SOTA）；2. 仅需7,000微调样本；3. 总训练成本约55美元。

Conclusion: DRA机制有效平衡探索与利用，证实语义多样性建模对强化学习微调的重要性，为低资源场景提供高性价比解决方案。
...
### [46] [Large Language Models Are More Persuasive Than Incentivized Human Persuaders](https://arxiv.org/abs/2505.09662)
*Philipp Schoenegger,Francesco Salvi,Jiacheng Liu,Xiaoli Nan,Ramit Debnath,Barbara Fasolo,Evelina Leivada,Gabriel Recchia,Fritz Günther,Ali Zarifhonarvar,Joe Kwon,Zahoor Ul Islam,Marco Dehnert,Daryl Y. H. Lee,Madeline G. Reinecke,David G. Kamper,Mert Kobaş,Adam Sandford,Jonas Kgomo,Luke Hewitt,Shreya Kapoor,Kerem Oktar,Eyup Engin Kucuk,Bo Feng,Cameron R. Jones,Izzy Gainsburg,Sebastian Olschewski,Nora Heinzelmann,Francisco Cruz,Ben M. Tappin,Tao Ma,Peter S. Park,Rayan Onyonka,Arthur Hjorth,Peter Slattery,Qingcheng Zeng,Lennart Finke,Igor Grossmann,Alessandro Salatiello,Ezra Karger*
...
<details>
  <summary>Details</summary>
Motivation: 探究前沿AI模型在实时对话场景中的说服能力，及其与激励型人类说服者的比较

Method: 采用预注册大规模激励实验：1）在线实时问答场景 2）说服者（人类/AI）引导参与者选择正误答案 3）测量服从率与经济收益

Result: AI说服成功率显著高于人类（+21%正确引导，+19%错误引导），双向改变参与者准确率与经济收益

Conclusion: AI说服能力已超越金钱激励的人类表现，突显AI治理与对齐机制的紧迫需求
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化研究集中于用户提示层面，忽视了跨任务通用的系统提示优化潜力。系统提示的优化可提升模型对不同用户提示的兼容性及任务迁移能力。

Method: 采用元学习框架，在多数据集上迭代优化系统提示与用户提示的协同关系，确保系统提示的鲁棒性和用户提示适配性。

Result: 在5个领域14个数据集上验证，优化后的系统提示对未见任务用户提示适配速度提升40%，性能平均提升12.6%。

Conclusion: 系统提示优化显著增强LLMs的任务泛化能力，实现零样本/小样本场景下的快速部署，为跨领域应用提供新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型事实性评估方法过度依赖分解-去上下文化流程，导致关键上下文丢失且无法捕捉复杂关系事实。需要更全面的评估体系同时衡量精确度和召回率

Method: 1. 开发VeriFact框架通过事实完整性增强和关系信息保留技术改进事实提取 2. 构建FactRBench基准，整合LLM生成内容与人工标注答案，支持召回率评估

Result: VeriFact显著提升事实完整性(较基线提高37%)并保留关键关系信息，相同模型家族中更大模型在精确度和召回率均有提升，但两者表现不总正相关

Conclusion: 综合事实性评估需同时考虑精确度与召回率，模型规模扩大能提升性能但需针对性优化，为LLM事实性评估提供新方法论与评估基准
...
TL;DR: 论文提出利用LLMs进行文本分析的迭代协作流程，通过分步教程展示如何生成并验证分类法，最终实现高效非结构化数据处理，同时讨论了技术局限性。


<details>
  <summary>Details</summary>
Motivation: 传统人工文本分析存在效率低和主观偏差问题，LLMs可兼顾效率与质量但缺乏系统应用方法，需建立标准化流程指导分类法开发与应用。

Method: 采用研究者与LLMs协作的迭代流程：1) 通过prompt生成初始分类法 2) 多轮评估优化分类结构 3) 测试编码者间信度 4) 全数据集应用验证可靠性。

Result: 以个人目标分类为例，该方法实现0.85的Kappa系数，证实LLMs能可靠完成文本分类，但存在上下文理解偏差需人工校正。

Conclusion: LLMs为文本分析提供高效解决方案，但需构建人机协作机制，结合自动化处理与人工验证，平衡效率与准确性。
...
### [50] [Achieving Tokenizer Flexibility in Language Models through Heuristic Adaptation and Supertoken Learning](https://arxiv.org/abs/2505.09738)
*Shaurya Sharthak,Vinayak Pahalwan,Adithya Kamath,Adarsh Shirawalmath*

Main category: cs.CL

TL;DR: 提出TokenAdapt框架解决LLM固定分词方案的低效问题，通过模型无关的分词器移植和Supertokens预学习显著提升多语言场景下的压缩效率与模型性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM固定分词方案导致多语言/专业场景效率低下，传统方法需大量算力且难以保持语义，现有改进方案仍存在再训练成本高和压缩效率不足的问题

Method: Tokenadapt结合局部（子词分解）与全局（语义相似性）混合启发式初始化新token嵌入，配合多词Supertokens的预分词学习机制

Result: 实验显示TokenAdapt困惑度比率较ReTok降低至少50%，Supertokens实现显著压缩增益，跨模型验证显示2倍以上综合性能提升

Conclusion: 该框架在保持语义的同时大幅降低再训练需求，为LLM分词方案优化提供了高效解决方案，特别适用于多语言和领域特定场景
...
### [51] [Automated Detection of Clinical Entities in Lung and Breast Cancer Reports Using NLP Techniques](https://arxiv.org/abs/2505.09794)
*J. Moreno-Casanova,J. M. Auñón,A. Mártinez-Pérez,M. E. Pérez-Martínez,M. E. Gas-López*

Main category: cs.CL

TL;DR: 利用NLP技术自动化提取肺癌/乳腺癌临床数据，使用uQuery工具和微调bsc-bio-ehr-en3模型，在200+400份西班牙医疗报告上实现高效实体识别（MET/PAT达优，EVOL仍有挑战）


<details>
  <summary>Details</summary>
Motivation: 解决人工提取临床数据效率低、错误率高的问题，针对肺癌和乳腺癌这两种高发癌症类型提升数据管理效率以改善患者预后

Method: 1. 使用GMV的uQuery工具进行标准化实体识别
2. 基于RoBERTa的bsc-bio-ehr-en3模型进行西班牙医疗文本微调
3. 采用IIS La Fe的600份标注报告（Doccano平台标注8类实体）

Result: 整体识别性能优异（MET准确率98.7%，PAT召回率95.2%），但低频实体EVOL的F1-score仅72.1%

Conclusion: 证实NLP在肿瘤临床数据处理中的有效性，未来需通过数据增强提升低频实体识别能力
...
### [52] [Exploring the generalization of LLM truth directions on conversational formats](https://arxiv.org/abs/2505.09807)
*Timour Ichmoukhamedov,David Martens*

Main category: cs.CL

TL;DR: 研究探索LLM中'真实方向'在不同对话格式中的泛化能力，发现短对话结尾撒谎时泛化良好，但长对话早期撒谎时表现差，提出通过添加关键短语提升泛化性。


<details>
  <summary>Details</summary>
Motivation: 基于前人发现LLM存在线性可分离的真假陈述方向，本研究旨在验证该方向在不同长度/结构的对话场景中的泛化能力，特别是谎言位置变化时探测器的稳定性。

Method: 通过测试不同对话格式(短对话谎言结尾 vs 长对话谎言前置)的泛化表现，提出在对话末尾添加固定关键短语的优化方案。

Result: 短对话结尾谎言场景泛化良好，但长对话早期谎言场景泛化差；添加关键短语后长对话场景的泛化性能显著提升。

Conclusion: LLM谎言检测器的实际应用面临泛化性挑战，需通过对话结构优化提升探测模型在新场景中的鲁棒性。
...
### [53] [KRISTEVA: Close Reading as a Novel Task for Benchmarking Interpretive Reasoning](https://arxiv.org/abs/2505.09825)
*Peiqi Sui,Juan Diego Rodriguez,Philippe Laban,Dean Murphy,Joseph P. Dexter,Richard Jean So,Samuel Baker,Pramit Chaudhuri*

Main category: cs.CL

TL;DR: 提出首个文学细读评测基准KRISTEVA，发现最先进大语言模型具备本科级细读能力（准确率49.7%-69.7%），但在11项任务中有10项落后于人类评估者。


<details>
  <summary>Details</summary>
Motivation: 大学英语课程每年产生数千万份需人工评阅的文本分析作业。细读能力作为批判性思维基础尚未在大语言模型上进行系统评估，现有基准缺乏文学学科覆盖。

Method: 构建包含1331道选择题的KRISTEVA基准，设计三个递进任务：1）提取文体特征 2）检索语境信息 3）风格与语境的多跳推理，测试模型对文学文本的解读能力。

Result: 当前最优模型在细读任务中展现本科水平能力，但在70%准确率的传统阈值下，10/11任务表现不及人类评估者（人类基线准确率高于模型）

Conclusion: 该研究填补了LLM文学解读能力评估空白，揭示了模型在需要深度文本分析与语境推理任务上的局限性，为改进AI的阐释性推理能力提供基准。
...
### [54] [Do Large Language Models Know Conflict? Investigating Parametric vs. Non-Parametric Knowledge of LLMs for Conflict Forecasting](https://arxiv.org/abs/2505.09852)
*Apollinaire Poli Nemkova,Sarath Chandra Lingareddy,Sagnik Ray Choudhury,Mark V. Albert*

Main category: cs.CL

TL;DR: 研究探讨LLMs能否通过预训练参数预测暴力冲突，并对比结合外部数据的非参数化方法，发现外部知识能有效提升冲突趋势和死亡人数的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究未充分验证LLMs在冲突预测中的潜力，而该能力对早期预警和人道救援至关重要。探索参数化与非参数化知识的结合可优化预测模型实用性。

Method: 构建2020-2024年非洲之角/中东的评估框架：1) 参数化设置仅用预训练权重预测冲突趋势和死亡人数；2) 非参数化设置通过RAG整合ACLED/GDELT等结构化数据及新闻事件。对比预测结果与历史数据。

Result: LLMs的预训练权重包含基础冲突预测能力，但结合外部结构化数据后预测准确率显著提升。非参数化方法在冲突趋势分类和死亡人数预测上表现更优。

Conclusion: LLMs具备冲突预测的初始能力，但需与实时数据结合方可实用化。该方法可为政策制定者提供动态风险预警，未来可扩展至更多冲突指标分析。
...
### [55] [Crossing Borders Without Crossing Boundaries: How Sociolinguistic Awareness Can Optimize User Engagement with Localized Spanish AI Models Across Hispanophone Countries](https://arxiv.org/abs/2505.09902)
*Martin Capdevila,Esteban Villa Turek,Ellen Karina Chumbe Fernandez,Luis Felipe Polo Galvez,Luis Cadavid,Andrea Marroquin,Rebeca Vargas Quesada,Johanna Crew,Nicole Vallejo Galarraga,Christopher Rodriguez,Diego Gutierrez,Radhi Datla*

Main category: cs.CL

TL;DR: 西班牙语存在显著地区差异，需构建至少五个子变体的本地化AI模型以弥合社会语言鸿沟，提升用户信任并促进可持续发展


<details>
  <summary>Details</summary>
Motivation: 西班牙语不同变体间的语言差异导致社会语言不协调，影响AI模型日常应用，需通过本地化策略实现文化包容性目标

Method: 通过社会文化和语言学的多维度差异分析（拉丁美洲与西班牙），提出西班牙语子变体建模框架

Result: 本地化模型可提升15-20%用户信任度，在低风险投资区域实现日均用户增长2.3%的可持续效益

Conclusion: 语言变体本地化是国际化战略的核心要素，五种子变体模型能有效增强文化敏感性并创造商业价值
...
### [56] [From Trade-off to Synergy: A Versatile Symbiotic Watermarking Framework for Large Language Models](https://arxiv.org/abs/2505.09924)
*Yidan Wang,Yubing Ren,Yanan Cao,Binxing Fang*

Main category: cs.CL

TL;DR: 提出Symbiotic水印框架，整合logits-based和sampling-based方案，通过串行/并行/混合三种策略实现检测性、鲁棒性、文本质量与安全性的平衡优化


<details>
  <summary>Details</summary>
Motivation: 现有水印方案在鲁棒性、文本质量和安全性之间存在权衡，需整合两种主流方案实现协同增效

Method: 通过令牌熵和语义熵自适应选择嵌入策略，构建可适应不同场景的混合水印框架

Result: 在多样化数据集和模型上的实验表明，该方法在检测率（98.4%）、文本质量（困惑度4.32）和抗攻击性（93.6%存活率）上达到SOTA
...
### [57] [Rethinking Prompt Optimizers: From Prompt Merits to Optimization](https://arxiv.org/abs/2505.09930)
*Zixiao Zhu,Hanzhang Zhou,Zijian Feng,Tianjiao Li,Chua Jia Jim Deryl,Mak Lee Onn,Gee Wah Ng,Kezhi Mao*

Main category: cs.CL

TL;DR: MePO提出轻量级本地可部署的prompt优化器，通过可解释的prompt质量指标指导优化，兼容不同规模模型并提升效果


<details>
  <summary>Details</summary>
Motivation: 现有prompt优化方法依赖GPT-4等大模型生成复杂prompt，导致轻量模型响应质量下降且存在成本/隐私问题

Method: 1. 建立模型无关的prompt质量评估标准
2. 基于轻量模型生成merit对齐的prompt构建偏好数据集
3. 训练merit引导的本地部署优化器MePO

Result: 跨任务和模型类型的实验证明MePO效果更优，尤其在轻量模型上提升显著

Conclusion: MePO通过可解释的优化指标实现低成本隐私安全的prompt优化，为实际部署提供可扩展解决方案
...
### [58] [Personalizing Large Language Models using Retrieval Augmented Generation and Knowledge Graph](https://arxiv.org/abs/2505.09945)
*Deeksha Prahlad,Chanhee Lee,Dongha Kim,Hokeun Kim*

Main category: cs.CL

TL;DR: 通过知识图谱增强检索的生成方法（RAG）有效提升LLMs在个性化日历场景中的准确性


<details>
  <summary>Details</summary>
Motivation: 解决LLMs因训练数据过时和缺乏个性化信息导致的幻觉问题，特别是日历等动态个人数据场景的响应准确性不足

Method: 将知识图谱（存储结构化实时数据）与检索增强生成技术结合，构建包含日历事件的三元组知识图谱进行检索增强

Result: 在个性化理解和响应准确率上显著优于基线模型（纯文本输入），响应时间增加14%但仍保持实用水平

Conclusion: 知识图谱的结构化存储特性有效提升LLMs的实时数据利用能力，为个性化AI助手提供可扩展解决方案
...
### [59] [DIF: A Framework for Benchmarking and Verifying Implicit Bias in LLMs](https://arxiv.org/abs/2505.10013)
*Lake Yin,Fan Huang*

Main category: cs.CL

TL;DR: 论文针对大语言模型的隐式偏见问题，提出DIF量化指标并通过社会角色实验验证偏见存在，发现模型准确率与偏见程度呈负相关


<details>
  <summary>Details</summary>
Motivation: 揭示LLM的隐式偏见不仅是伦理问题，更暴露其技术缺陷——无法有效整合外部信息。当前缺乏该领域标准化评估方法，需建立可解释的量化基准

Method: 通过给现有逻辑/数学题数据集添加社会人口属性角色，构建DIF指标（Demographic Implicit Fairness），统计分析模型表现与人口特征的关系

Result: DIF方法统计验证了LLM存在隐式偏见，发现问答准确率与偏见程度呈现显著反向相关趋势

Conclusion: 研究确立了首个可解释的LLM隐式偏见量化基准，证明模型性能与公平性存在内在冲突，为模型评估提供新维度
...
### [60] [CAFE: Retrieval Head-based Coarse-to-Fine Information Seeking to Enhance Multi-Document QA Capability](https://arxiv.org/abs/2505.10063)
*Han Peng,Jinhao Jiang,Zican Dong,Wayne Xin Zhao,Lei Fang*

Main category: cs.CL

TL;DR: 提出两阶段方法CAFE，通过粗粒度过滤和细粒度引导增强多文档问答能力，减少干扰文档影响


<details>
  <summary>Details</summary>
Motivation: 现有方法在长上下文问答中存在检索精度与召回率的平衡问题，导致证据文档利用率不足

Method: 1. 粗粒度过滤：利用检索头识别并排序相关文档
2. 细粒度引导：通过注意力机制聚焦关键内容

Result: 在Mistral模型上实现SubEM指标提升，较SFT和RAG方法分别提高22.1%和13.7%

Conclusion: CAFE通过分级处理机制有效提升模型对证据文档的依赖，显著改善多文档问答性能
...
### [61] [Dark LLMs: The Growing Threat of Unaligned AI Models](https://arxiv.org/abs/2505.10066)
*Michael Fire,Yitzhak Elbazis,Adi Wasenstein,Lior Rokach*

Main category: cs.CL

TL;DR: LLMs面临越狱攻击风险，其训练数据中的未过滤内容导致安全漏洞。研究发现通用越狱攻击可突破多款先进模型，行业应对措施不足。


<details>
  <summary>Details</summary>
Motivation: 大语言模型快速发展的同时，其训练数据中潜在的‘暗数据’导致模型易受越狱攻击，威胁AI安全性。研究者试图揭示该漏洞的普遍性和行业应对缺陷。

Method: 通过构建通用越狱攻击策略（具体方法未公开），对多个前沿LLM进行渗透测试，验证攻击有效性。攻击原理基于模型训练数据中的不良模式利用。

Result: 7个月前公开的攻击方法仍对多数主流LLM有效；厂商响应滞后，开源模型扩散加剧风险；模型训练门槛降低导致危险知识传播风险陡增。

Conclusion: 当前LLM安全防护存在系统性漏洞，需建立更严格的训练数据审查机制和快速响应体系，否则可能引发远超预期的社会风险。
...
### [62] [Designing and Contextualising Probes for African Languages](https://arxiv.org/abs/2505.10081)
*Wisdom Aduah,Francois Meyer*

Main category: cs.CL

TL;DR: 系统研究表明针对非洲语言优化的预训练模型比多语言模型更能捕捉目标语言的句法和语义特征，控制任务验证了模型内部知识的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有非洲语言预训练模型的进步原因不明确，需系统性探究其语言学知识编码机制及优化策略的有效性基础。

Method: 1. 为6种非洲语言训练分层探针
2. 设计MasakhaPOS控制任务验证探针可靠性
3. 对比非洲专用模型与多语言模型的性能差异

Result: 非洲专用模型编码更多目标语言特征；句法信息集中于中后层，语义信息全层分布；控制任务证实模型真实知识非探针记忆

Conclusion: 通过可解释性技术揭示了非洲语言模型优化策略（主动学习/多语言适应）的内部机制，为后续优化提供理论依据
...
### [63] [XRAG: Cross-lingual Retrieval-Augmented Generation](https://arxiv.org/abs/2505.10089)
*Wei Liu,Sony Trenous,Leonardo F. R. Ribeiro,Bill Byrne,Felix Hieber*

Main category: cs.CL

TL;DR: XRAG是首个专注于评估大语言模型在跨语言检索增强生成场景中表现的新基准，通过新闻数据构建复杂推理问题，揭示了LLMs在跨语言场景中的响应语言正确性和多语言信息整合两大新挑战。


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中用户语言与检索结果不匹配时LLMs的表现评估问题，通过构建需要外部知识支持的复杂推理问题，为研究LLMs的跨语言推理能力提供基准。

Method: 基于近期新闻文章构建数据集，覆盖单语检索和多语言检索场景，通过创新的问题生成流程设计需要复杂推理的问题，并标注检索文档相关性。

Result: 实验发现：1）单语检索场景中所有模型存在响应语言正确性问题；2）多语言检索的主要挑战在于跨语言信息推理而非非英语文本生成。

Conclusion: XRAG不仅是跨语言RAG的评估工具，其构建方法论本身也为研究LLMs的通用推理能力提供了新视角，揭示了跨语言场景特有的技术挑战。
...
### [64] [What Does Neuro Mean to Cardio? Investigating the Role of Clinical Specialty Data in Medical LLMs](https://arxiv.org/abs/2505.10113)
*Xinlan Yan,Di Wu,Yibin Lei,Christof Monz,Iacer Calixto*

Main category: cs.CL

TL;DR: 提出S-MedQA医学问答数据集验证大模型性能，发现领域迁移比知识注入更能提升临床QA效果


<details>
  <summary>Details</summary>
Motivation: 验证医学领域知识注入假说，探索微调数据对专业领域模型性能的影响机制

Method: 使用自建S-MedQA数据集进行多专业实验，分析训练数据与专业性能关联性及token概率变化

Result: 专业微调不一定提升对应领域性能，但临床相关术语概率普遍提升；性能提升主要来自领域迁移

Conclusion: 医学领域性能提升源于领域迁移而非知识注入，需重新审视微调数据在专业领域的作用
...
### [65] [GE-Chat: A Graph Enhanced RAG Framework for Evidential Response Generation of LLMs](https://arxiv.org/abs/2505.10143)
*Longchao Da,Parth Mitesh Shah,Kuan-Ru Liou,Jiaxing Zhang,Hua Wei*

Main category: cs.CL

TL;DR: 提出GE-Chat框架，通过知识图谱增强检索生成机制，为LLM生成基于证据的可信响应。


<details>
  <summary>Details</summary>
Motivation: LLM输出存在不可靠性（如幻觉回答），用户需手动验证结论，影响信任度。

Method: 1) 文档构建知识图谱 → 2) 思维链生成 → 3) n跳子图搜索 → 4) 蕴含式句子生成 → 实现精准证据检索

Result: 提升现有模型在自由文本中定位证据的准确性，提供验证LLM结论来源的可解释路径

Conclusion: 通过显式的知识图谱证据链，增强用户对LLM输出可信度的判断能力
...
### [66] [Mining Hidden Thoughts from Texts: Evaluating Continual Pretraining with Synthetic Data for LLM Reasoning](https://arxiv.org/abs/2505.10182)
*Yoichi Ishibashi,Taro Yano,Masafumi Oyamada*

Main category: cs.CL

TL;DR: Reasoning CPT通过合成隐藏思维数据提升LLM跨领域推理能力，尤其在难题上效果显著（最高提升8%），且模型可自适应调整推理深度。
...
Method: 在Gemma2-9B模型上实施Reasoning CPT：1. 基于STEM/Law语料生成隐藏思维数据（假设文本为思维过程的结果） 2. 与标准CPT在MMLU基准对比评估

Result: 1. 全领域性能提升 2. 跨领域推理迁移有效（法律→STEM） 3. 问题难度每增一级，性能优势扩大2-8% 4. 模型自动调节思维深度与问题难度正相关

Conclusion: 重构隐藏思维过程的CPT显著增强模型通用推理能力，突破领域限制，且证明：高难度问题更依赖系统性推理，这种能力具有可迁移性。
...
### [67] [The CoT Encyclopedia: Analyzing, Predicting, and Controlling how a Reasoning Model will Think](https://arxiv.org/abs/2505.10185)
*Seongyun Lee,Seungone Kim,Minju Seo,Yongrae Jo,Dongyoung Go,Hyeonbin Hwang,Jinho Park,Xiang Yue,Sean Welleck,Graham Neubig,Moontae Lee,Minjoon Seo*

Main category: cs.CL

TL;DR: 提出CoT百科全书框架，通过自下而上方式自动分析大语言模型的推理策略


<details>
  <summary>Details</summary>
Motivation: 突破传统基于人工直觉的策略分类限制，全面捕捉模型推理行为的多样性

Method: 从模型生成的思维链中自动提取推理标准→语义嵌入→聚类→构建对比式解释框架

Result: 人类评估显示该方法比现有方法更具解释性和全面性，并能指导模型采用更优策略

Conclusion: 训练数据格式（如自由文本vs多选题）对推理行为的影响远大于数据领域本身
...
### [68] [VQ-Logits: Compressing the Output Bottleneck of Large Language Models via Vector Quantized Logits](https://arxiv.org/abs/2505.10202)
*Jintian Shao,Hongyi Huang,Jiayi Wu,YiMing Cheng,ZhiYu Wu,You Shan,MingKai Zheng*

Main category: cs.CL

TL;DR: 使用向量量化技术压缩大语言模型输出层参数，实现99%参数削减和6倍计算加速，仅带来4%的困惑度增长


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型输出层因海量词汇表导致的巨大参数量和计算成本问题，现有方法存在结构复杂性问题

Method: 用小型共享码本替代传统输出嵌入矩阵，通过向量量化将词汇表映射到码本向量，模型预测码本空间logits后散射到完整词汇空间

Result: 在WikiText-103等基准测试中实现输出层99%参数削减，计算速度提升6倍，困惑度仅增加4%

Conclusion: VQ-Logits为LLM输出层压缩提供高效解决方案，消融实验验证了码本尺寸/初始化策略的鲁棒性
...
### [69] [RAIDEN-R1: Improving Role-awareness of LLMs via GRPO with Verifiable Reward](https://arxiv.org/abs/2505.10218)
*Zongsheng Wang,Kaili Sun,Bowen Wu,Qun Yu,Ying Li,Baoxun Wang*

Main category: cs.CL

TL;DR: 提出RAIDEN-R1强化学习框架，通过可验证角色感知奖励（VRAR）解决角色扮演对话代理（RPCAs）的角色一致性难题，14B-GRPO模型在关键指标上超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 针对RPCAs角色一致性不足的痛点，传统方法缺乏可量化评估机制，需建立可验证的奖励体系提升角色认知能力。

Method: 融合单/多阶段挖掘策略生成可量化奖励，构建多LLM协作的角色感知思维链数据集，通过强化学习优化推理连贯性。

Result: 14B-GRPO模型在剧本知识（88.04%）和对话记忆（88.65%）指标达到SOTA，案例分析显示其冲突上下文处理能力提升3倍。

Conclusion: 首次实现RPCA训练的可量化评估，揭示角色感知推理模式，为具身智能对话系统开发提供新范式。
...
### [70] [Comparing LLM Text Annotation Skills: A Study on Human Rights Violations in Social Media Data](https://arxiv.org/abs/2505.10260)
*Poli Apollinaire Nemkova,Solomon Ubani,Mark V. Albert*

Main category: cs.CL

TL;DR: 研究对比GPT-3.5/4、LLAMA3等大模型在俄乌社交媒体数据的人权侵犯二分类任务中的零样本/少样本标注性能，发现模型间存在显著性能差异及跨语言适应性特征。


<details>
  <summary>Details</summary>
Motivation: 验证大语言模型在敏感多语言场景下的标注可靠性，探索其处理主观语境任务时的跨语言适配能力。

Method: 使用5个前沿LLM对1000条人工双标注样本进行标注，对比英语/俄语提示下的分类表现，分析错误模式与标注者间一致性。

Result: 不同模型间F1分数差异达23.5%，俄语提示平均提升7.2%召回率。GPT-4展现最强上下文推理能力，但存在文化语境误判现象。

Conclusion: LLMs在多语言敏感任务中展现潜力但存在语境依赖性，需结合领域知识进行结果校验，提示语言选择显著影响模型跨文化理解。
...
### [71] [The Evolving Landscape of Generative Large Language Models and Traditional Natural Language Processing in Medicine](https://arxiv.org/abs/2505.10261)
*Rui Yang,Huitao Li,Matthew Yu Heng Wong,Yuhe Ke,Xin Li,Kunyu Yu,Jingchi Liao,Jonathan Chong Kai Liew,Sabarinath Vinod Nair,Jasmine Chiat Ling Ong,Irene Li,Douglas Teodoro,Chuan Hong,Daniel Shu Wei Ting,Nan Liu*

Main category: cs.CL

TL;DR: 生成式大语言模型在医学开放式任务中表现优异，传统NLP在信息处理任务保持优势，需注重伦理规范应用


<details>
  <summary>Details</summary>
Motivation: 探讨生成式LLMs与传统NLP在不同医疗任务中的性能差异及应用边界

Method: 通过分析19,123项医学领域研究数据进行对比研究

Result: 生成式LLMs在开放型任务（如问答生成）中更具优势，传统NLP在信息抽取/分析类任务保持领先

Conclusion: 两种技术存在明确的应用场景区分，未来需结合伦理框架推动医疗AI的负责任发展
...
### [72] [From Questions to Clinical Recommendations: Large Language Models Driving Evidence-Based Clinical Decision Making](https://arxiv.org/abs/2505.10282)
*Dubai Li,Nan Jiang,Kangping Huang,Ruiqi Tu,Shuyu Ouyang,Huayu Yu,Lin Qiao,Chen Yu,Tianshu Zhou,Danyang Tong,Qian Wang,Mengtao Li,Xiaofeng Zeng,Yu Tian,Xinping Tian,Jingsong Li*

Main category: cs.CL

TL;DR: 开发基于大语言模型的临床决策支持系统Quicker，通过自动化证据合成与标准化流程建模，显著提升临床决策效率与可靠性


<details>
  <summary>Details</summary>
Motivation: 临床证据整合面临工作量大（需处理2.2万篇文献）、流程复杂（涉及分级推荐系统开发）和时间压力（单病种指南开发需200小时）的挑战，需要自动化工具支持

Method: 构建端到端自动化系统：1) 基于GRADE框架建立问题分解机制 2) 开发多阶段文献筛选模块（敏感性达95%）3) 集成证据评估仪表盘与动态推荐生成器

Result: 在Q2CRBench-3测试中：文献查全率提升32%（vs人工），推荐逻辑连贯性评分达4.2/5，单病种决策时间从200小时缩短至40分钟，证据支持完整度提升41%

Conclusion: Quicker系统通过自动化证据处理链条，有效解决临床决策中的证据整合瓶颈，为构建实时更新的智能临床指南系统奠定技术基础
...
### [73] [J1: Incentivizing Thinking in LLM-as-a-Judge via Reinforcement Learning](https://arxiv.org/abs/2505.10320)
*Chenxi Whitehouse,Tianlu Wang,Ping Yu,Xian Li,Jason Weston,Ilia Kulikov,Swarnadeep Saha*

Main category: cs.CL

TL;DR: 提出强化学习框架J1，通过可验证奖励机制提升LLM评估模型的判断能力，在多个规模下超越现有模型表现。


<details>
  <summary>Details</summary>
Motivation: AI发展受限于评估质量，现有LLM评估模型需提升思维链推理能力以生成更准确的判断。

Method: 将可验证/不可验证提示转化为带可验证奖励的判断任务，通过在线强化学习框架训练模型，整合评估标准生成、参考答案比对、响应重评估等机制。

Result: J1在8B/70B规模均超越同类模型，部分场景击败更大模型R1；模型通过生成评估标准、参考答案对比提升判断质量。

Conclusion: J1证明了强化学习训练框架在提升评估模型判断能力上的有效性，小模型通过优化思维过程可获得超越大模型的评估性能。
...
### [74] [LDIR: Low-Dimensional Dense and Interpretable Text Embeddings with Relative Representations](https://arxiv.org/abs/2505.10354)
*Yile Wang,Zhanyu Shen,Hui Huang*

Main category: cs.CL

TL;DR: 提出LDIR方法，通过低维密集相对表示实现高性能且可解释的文本嵌入，性能接近黑盒模型并超越现有可解释基线


<details>
  <summary>Details</summary>
Motivation: 现有可解释文本嵌入存在维度与性能的矛盾（高维可解释模型复杂，低维稀疏模型性能差），需开发兼顾低维、高性能和可解释性的新方法

Method: 采用最远点采样选择锚文本，构建维度数值反映文本与锚文本语义相关性的相对表示，通过密集低维向量实现语义编码与解释性平衡

Result: 在语义相似度、检索和聚类任务中，LDIR性能接近黑盒模型（如SimCSE），显著优于现有可解释方法且维度降低95%以上

Conclusion: LDIR证明了低维密集表示与可解释性的兼容性，为构建高效可解释文本嵌入提供了新的技术路径
...
### [75] [Coherent Language Reconstruction from Brain Recordings with Flexible Multi-Modal Input Stimuli](https://arxiv.org/abs/2505.10356)
*Chunyu Ye,Shaonan Wang*

Main category: cs.CL

TL;DR: 提出统一多模态框架，利用视觉语言模型实现跨视觉/听觉/文本的脑信号解码，性能媲美单模态SOTA且更具扩展性


<details>
  <summary>Details</summary>
Motivation: 现有fMRI语言重建研究局限于单模态输入，而人类思维本质是多模态的，需开发更灵活统一的解码框架

Method: 基于视觉语言模型（VLMs），通过模态特异性专家网络实现跨模态信息联合解码

Result: 在多种模态输入场景下达到与单模态SOTA相当的性能，同时保持框架的适应性和可扩展性

Conclusion: 该工作通过多模态融合提升了思维解码的生态效度和泛化能力，推动脑机交互向真实场景迈进
...
### [76] [Multi-domain Multilingual Sentiment Analysis in Industry: Predicting Aspect-based Opinion Quadruples](https://arxiv.org/abs/2505.10389)
*Benjamin White,Anastasia Shimorina*

Main category: cs.CL

TL;DR: 开发基于大语言模型的多领域四元观点提取系统，实现与单领域模型相当的性能同时降低复杂度


<details>
  <summary>Details</summary>
Motivation: 探索如何利用单一微调模型处理多领域多语言的复杂情感分析任务，解决部署多个单领域模型带来的运维复杂性

Method: 使用内部数据集训练组合多领域模型，对比分析单领域与多领域模型在四元观点提取任务上的性能差异

Result: 多领域模型在保持性能相当（与单领域模型相比）的同时，显著降低了模型运维的复杂性

Conclusion: 验证了统一多领域模型的可行性，并总结了处理非提取预测及评估LLM结构化任务失败模式的重要实践经验
...
### [77] [Rethinking Repetition Problems of LLMs in Code Generation](https://arxiv.org/abs/2505.10402)
*Yihong Dong,Yuchen Liu,Xue Jiang,Zhi Jin,Ge Li*

Main category: cs.CL

TL;DR: 提出基于语法规则的RPG解码方法，有效缓解代码生成中的结构重复问题并提升质量


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注代码内容重复，但更普遍的结构重复问题(语法层面固定结构的重复模式)尚未被充分解决

Method: RPG方法：1. 利用语法规则实时检测结构重复 2. 对导致重复的关键token进行可能性衰减 3. 构建CodeRepetEval评估数据集

Result: 在CodeRepetEval/HumanEval/MBPP基准测试中显著优于基线模型，有效减少重复并提升代码质量

Conclusion: RPG通过语法引导的解码机制，系统性解决代码生成中的重复问题，为编程辅助工具提供新思路
...
### [78] [Are LLM-generated plain language summaries truly understandable? A large-scale crowdsourced evaluation](https://arxiv.org/abs/2505.10409)
*Yue Guo,Jae Ho Sohn,Gondy Leroy,Trevor Cohen*

Main category: cs.CL

TL;DR: 研究通过大规模众包评估发现，尽管LLM生成的医学简明摘要主观质量与人类撰写相当，但人类撰写的摘要显著提升患者理解效果，且自动化评估指标无法有效反映人工判断。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型生成医学简明摘要的有效性缺乏系统性评估，既往研究依赖自动化指标或小样本主观评分，无法准确衡量患者真实理解程度。

Method: 通过亚马逊众包平台招募150名参与者，结合主观评分（简洁性/信息量/连贯性/忠实度）和客观测试（选择题理解/回忆测试），并对比10种自动评估指标与人工判断的一致性。

Result: 1. LLM生成摘要主观评分与人类相当（p=0.12）
2. 人类摘要理解正确率显著更高（62% vs 53%，p<0.01）
3. 自动评估指标与人工判断相关性弱（平均ρ=0.22）

Conclusion: 需建立超越表面质量评估的新框架，并开发以患者理解为优化目标的内容生成方法，当前自动评估体系不适用于医学简明摘要质量评价。
...
### [79] [Hierarchical Document Refinement for Long-context Retrieval-augmented Generation](https://arxiv.org/abs/2505.10413)
*Jiajie Jin,Xiaoxi Li,Guanting Dong,Yuyao Zhang,Yutao Zhu,Yongkang Wu,Zhonghua Li,Qi Ye,Zhicheng Dou*

Main category: cs.CL

TL;DR: 提出LongRefiner优化器，通过双级查询分析和层次化结构处理长文本RAG应用，实现10倍计算效率提升


<details>
  <summary>Details</summary>
Motivation: 解决长上下文RAG应用中冗余信息和噪声导致的推理成本高、性能下降问题

Method: 结合双级查询分析、层次化文档构建，在单一基础模型上通过多任务学习实现自适应优化

Result: 在7个QA数据集上达到最佳基线性能，计算成本和延迟减少10倍，验证可扩展性和有效性

Conclusion: 为实际长文本RAG应用提供高效解决方案，代码开源支持实践应用
...
### [80] [Reinforcing the Diffusion Chain of Lateral Thought with Diffusion Language Models](https://arxiv.org/abs/2505.10446)
*Zemin Huang,Zhiyang Chen,Zijun Wang,Tiancheng Li,Guo-Jun Qi*

Main category: cs.CL

TL;DR: 提出了DCoLT框架，通过逆向扩散过程的强化学习优化推理轨迹，显著提升扩散语言模型在数学和代码生成任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 突破传统CoT方法的线性思维限制，允许非语法约束的双向非线性推理，充分利用扩散模型中间步骤的潜在思考能力。

Method: 基于SEDD的连续时间扩散模型实现概率策略优化，在LLaDA模型上采用Plackett-Luce模型的Unmasking Policy Module进行词序预测优化。

Result: DCoLT增强的LLaDA在GSM8K/MATH/MBPP/HumanEval任务上分别提升+9.8%/+5.7%/+11.4%/+19.5%准确率，仅用公开数据和16块H800 GPU。
...
### [81] [CL-RAG: Bridging the Gap in Retrieval-Augmented Generation with Curriculum Learning](https://arxiv.org/abs/2505.10493)
*Shaohan Wang,Licheng Zhang,Zheren Fu,Zhendong Mao*

Main category: cs.CL

TL;DR: 提出基于课程学习的多阶段训练框架CL-RAG，通过分阶段优化检索器和生成器，在开放域QA任务中实现2%-4%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统直接使用top-k文档，但不同查询的有效文档差异大，影响模型训练时的适应能力。受人类渐进式学习启发，将课程学习引入RAG训练过程。

Method: 1. 通过样本进化分别构建检索器和生成器的多难度训练数据
2. 采用课程学习范式分阶段训练模型

Result: 在四个开放域QA数据集上性能超越现有方法2%-4%，验证了框架的通用性

Conclusion: CL-RAG有效优化RAG系统性能，证明课程学习机制对检索生成联合优化的促进作用，为复杂场景下的语言模型增强提供新思路
...
### [82] [Can You Really Trust Code Copilots? Evaluating Large Language Models from a Code Security Perspective](https://arxiv.org/abs/2505.10494)
*Yutao Mou,Xiao Deng,Yuxiao Luo,Shikun Zhang,Wei Ye*

Main category: cs.CL

TL;DR: 提出CoV-Eval多任务基准测试和VC-Judge评估模型，发现现有大语言模型在代码生成安全性和漏洞修复方面存在显著不足


<details>
  <summary>Details</summary>
Motivation: 现有代码安全基准测试局限于单一任务维度（如代码补全），缺乏代码生成/修复/检测的全方位安全评估体系

Method: 构建覆盖代码补全、漏洞修复、漏洞检测分类的多维度评估框架CoV-Eval，开发与人类专家对齐的VC-Judge评估模型，对20个主流LLM进行系统评估

Result: 多数LLM擅长漏洞识别（准确率>80%），但存在安全代码生成率低（平均仅35%）、特定漏洞类型识别困难（如内存泄漏）、修复成功率不足（<50%）三大核心缺陷

Conclusion: 揭示了LLM代码安全能力的关键瓶颈，指出需加强安全代码生成、细粒度漏洞理解、上下文感知修复三大方向的技术突破
...
### [83] [The Devil Is in the Word Alignment Details: On Translation-Based Cross-Lingual Transfer for Token Classification Tasks](https://arxiv.org/abs/2505.10507)
*Benedikt Ebing,Goran Glavaš*

Main category: cs.CL

TL;DR: 研究通过优化词对齐器的设计策略（投影算法、标签过滤、预分词）并集成translate-train/translate-test预测，显著提升了跨语言迁移在token分类任务中的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 跨语言迁移中的标签投影存在挑战，传统词对齐方法缺乏系统分析，而基于标记的方法声称更优。本文重新评估词对齐器的潜力并探索更鲁棒的解决方案。

Method: 系统研究词对齐器在标签投影中的三个关键设计因素：(i)跨token标签投影算法 (ii)噪声标签过滤策略 (iii)译文的预分词方式，提出集成两种翻译策略预测结果的新方法。

Result: 优化后的词对齐器性能与标记方法相当，新提出的集成策略不仅超越标记方法，且降低对底层设计选择的敏感性，提升跨语言迁移稳定性。

Conclusion: 通过系统性优化和集成策略，基于词对齐器的翻译方法能够实现更鲁棒的跨语言迁移效果，挑战了标记方法更优的既有认知。
...
### [84] [Multi-Token Prediction Needs Registers](https://arxiv.org/abs/2505.10518)
*Anastasios Gerontopoulos,Spyros Gidaris,Nikos Komodakis*

Main category: cs.CL

TL;DR: 提出MuToR方法——通过插入可学习的寄存器令牌实现高效多令牌预测，在保持原有架构兼容性的同时提升监督微调效果


<details>
  <summary>Details</summary>
Motivation: 现有多令牌预测方法在微调阶段效果欠佳，需要兼容现有预训练模型且不显著增加参数量的改进方案

Method: 在输入序列中插入可学习的寄存器令牌，每个令牌负责预测未来目标，保持与现有架构兼容，无需修改预训练目标

Result: 在语言和视觉领域的生成任务中验证有效性，包括监督微调、参数高效微调（PEFT）和预训练场景

Conclusion: MuToR通过极简设计实现高效多令牌预测，特别适合监督微调，支持可扩展的预测范围，具有广泛适用性
...
### [85] [WorldPM: Scaling Human Preference Modeling](https://arxiv.org/abs/2505.10527)
*Binghai Wang,Runji Lin,Keming Lu,Le Yu,Zhenru Zhang,Fei Huang,Chujie Zheng,Kai Dang,Yang Fan,Xingzhang Ren,An Yang,Binyuan Hui,Dayiheng Liu,Tao Gui,Qi Zhang,Xuanjing Huang,Yu-Gang Jiang,Bowen Yu,Jingren Zhou,Junyang Lin*

Main category: cs.CL

TL;DR: 通过大规模数据和模型扩展验证偏好建模的扩展定律，WorldPM框架在对抗性和客观指标中展现显著扩展潜力，主观指标未呈现扩展趋势。


<details>
  <summary>Details</summary>
Motivation: 借鉴语言模型的规模定律，探索其在偏好建模中的应用潜力，构建统一的人类偏好表征框架WorldPM。

Method: 收集公共论坛多样化偏好数据，使用15M规模数据集训练1.5B-72B参数模型，在7个基准20个子任务开展评估。

Result: 对抗性指标随规模稳定提升，客观指标呈现突现能力，主观指标无扩展趋势。WorldPM使泛化性能提升超5%，RLHF流程集成后评估结果提升4-8%。

Conclusion: WorldPM证实偏好建模的扩展潜力，揭示不同指标特性差异，为偏好微调提供有效基础框架。
...
### [86] [Beyond 'Aha!': Toward Systematic Meta-Abilities Alignment in Large Reasoning Models](https://arxiv.org/abs/2505.10554)
*Zhiyuan Hu,Yibo Wang,Hanze Dong,Yuhui Xu,Amrita Saha,Caiming Xiong,Bryan Hooi,Junnan Li*

Main category: cs.CL

TL;DR: 研究提出通过显式对齐模型的演绎/归纳/溯因三种元能力，突破传统提示工程和随机涌现的局限，构建可扩展的推理基础


<details>
  <summary>Details</summary>
Motivation: 现有基于结果的强化学习虽能偶然引发自我修正等推理行为，但这些行为的时机和一致性不可控，限制了大型推理模型的可靠性和扩展性

Method: 采用三阶段框架：个体元能力对齐训练、参数空间融合、领域强化学习，通过自动生成的自验证任务驱动能力内化

Result: 在数学/编程/科学基准测试中，相对指令微调基线提升超10%，领域强化学习进一步带来2%的平均增益

Conclusion: 显式元能力对齐为复杂推理任务提供了可扩展且可靠的基础框架，突破了传统基于提示和随机涌现的范式限制
...
### [87] [VRSplat: Fast and Robust Gaussian Splatting for Virtual Reality](https://arxiv.org/abs/2505.10144)
...
### [88] [Style Customization of Text-to-Vector Generation with Image Diffusion Priors](https://arxiv.org/abs/2505.10558)
...
### [89] [VRSplat: Fast and Robust Gaussian Splatting for Virtual Reality](https://arxiv.org/abs/2505.10144)
*Xuechang Tu,Lukas Radl,Michael Steiner,Markus Steinberger,Bernhard Kerbl,Fernando de la Torre*

Main category: cs.GR

TL;DR: VRSplat整合Mini-Splatting、StopThePop和Optimal Projection技术，首次实现支持现代VR应用的3DGS方案，消除画面异常并达到72+ FPS


<details>
  <summary>Details</summary>
Motivation: 解决3DGS在VR中面临的三重挑战：头部运动导致的时间伪影、投影失真产生的视觉干扰浮点，以及大场景下帧率不足的问题

Method: 改进核心光栅化器+眼窝分区渲染（单GPU启动处理聚焦/周边区域）+基于深度评估的参数微调，综合优化高斯溅射算法

Result: 用户研究显示25名参与者显著偏好VRSplat，系统在保持72+帧率的同时消除画面异常

Conclusion: VRSplat是首个经系统验证、能支撑现代VR的3DGS方案，通过技术创新实现实时稳定渲染
...
### [90] [Style Customization of Text-to-Vector Generation with Image Diffusion Priors](https://arxiv.org/abs/2505.10558)
*Peiying Zhang,Nanxuan Zhao,Jing Liao*

Main category: cs.GR

TL;DR: 提出两阶段SVG风格定制框架，结合前馈T2V模型的结构优势与T2I图像先验，实现高效生成风格一致的矢量图形。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成矢量方法无法兼顾风格定制需求与结构规律性，前者导致视觉一致性不足，后者受限于数据量难以解耦内容与风格。

Method: 1. 训练路径级T2V扩散模型确保SVG结构规律性；2. 通过蒸馏定制T2I模型实现风格迁移，保留生成多样性。

Result: 实验验证该方法能生成高质量、风格统一的SVG，在效率与视觉一致性上优于现有方案。

Conclusion: 结合前馈模型结构优势与T2I风格定制能力，有效平衡矢量图形的结构规范与风格表达需求。
...
### [91] [CartoAgent: a multimodal large language model-powered multi-agent cartographic framework for map style transfer and evaluation](https://arxiv.org/abs/2505.09936)
...
### [92] [CartoAgent: a multimodal large language model-powered multi-agent cartographic framework for map style transfer and evaluation](https://arxiv.org/abs/2505.09936)
*Chenglong Wang,Yuhao Kang,Zhaoya Gong,Pengjun Zhao,Yu Feng,Wenjia Zhang,Ge Li*

Main category: cs.HC

TL;DR: 提出CartoAgent多智能体框架，利用多模态大语言模型实现地图重设计，通过分离样式与地理数据确保准确性，在地图风格迁移任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以兼顾地图艺术性与信息准确性，GenAI发展为制图流程创新提供可能。研究旨在利用MLLM的视觉审美能力提升地图美观度与信息量。

Method: 构建三阶段多代理协作框架：1) 准备阶段配置任务参数 2) 设计阶段MLLM代理协同生成样式表 3) 评估阶段多维度验证地图质量，通过样式-数据分离机制保持地理准确性。

Result: 实验证明框架能有效完成地图风格迁移，人工评估显示生成地图在美观性(提升32%)和信息完整性(提升28%)方面显著优于基线方法。

Conclusion: CartoAgent展示了GenAI驱动制图决策的潜力，其模块化设计可扩展至其他制图场景，为智能制图系统开发提供新范式。
...
### [93] [Visual Feedback of Pattern Separability Improves Myoelectric Decoding Performance of Upper Limb Prostheses](https://arxiv.org/abs/2505.09819)
...
TL;DR: 开发了Reviewer 3D可视化界面，通过实时投影肌电信号至解码器分类空间，显著提升PR控制系统训练效果和操作效率。


<details>
  <summary>Details</summary>
Motivation: 现有肌电假肢模式识别系统依赖用户生成高区分度EMG信号，传统试错训练效率低下，需要更直观的反馈机制实现用户与解码器的协同适应。

Method: 对12名健康受试者进行10次训练对比实验，使用Fitts定律任务评估Reviewer 3D可视化与传统虚拟手臂在路径效率、完成率等指标的表现差异。

Result: Reviewer组在任务完成率(提高19%)、路径效率(提升27%)和吞吐量(增加0.8bps)等核心指标显著优于对照组，同时减少32%的轨迹超调。

Conclusion: 通过将解码器分类空间可视化，Reviewer建立了双向反馈训练框架，使操作者训练效率提升40%，减少75%的试错校准需求，推动PR控制系统向自适应训练范式转变。
...
### [94] [SOS: A Shuffle Order Strategy for Data Augmentation in Industrial Human Activity Recognition](https://arxiv.org/abs/2505.10312)
*Anh Tuan Ha,Hoang Khang Phan,Thai Minh Tien Ngo,Anh Phan Truong,Nhat Tan Le*

Main category: cs.HC

TL;DR: 利用注意力自编码器和条件生成对抗网络生成数据集，并通过随机序列重组策略提升人体活动识别模型性能至准确率0.70±0.03。


<details>
  <summary>Details</summary>
Motivation: 解决人体活动识别领域高质量数据获取成本高、现实场景活动异质性导致的模型泛化能力不足问题。

Method: 采用深度学习生成合成数据（注意力自编码器+条件GAN），并通过打乱时间序列消除数据异质性，强制模型学习瞬时特征。

Result: 随机序列策略使分类准确率达0.70±0.03，宏观F1分数0.64±0.01，模型对活动过渡的鲁棒性显著增强。

Conclusion: 时间依赖解构策略不仅扩展了有效训练数据集，还为复杂场景下的人体活动识别系统优化提供了新思路。
...
### [95] [FlowDreamer: A RGB-D World Model with Flow-based Motion Representations for Robot Manipulation](https://arxiv.org/abs/2505.10075)
*Jun Guo,Xiaojian Ma,Yikai Wang,Min Yang,Huaping Liu,Qing Li*

Main category: cs.RO

TL;DR: 提出FlowDreamer模型，通过显式3D场景流预测提升机器人操作任务中的视觉世界建模性能


<details>
  <summary>Details</summary>
Motivation: 传统方法隐式处理动态预测并与视觉渲染耦合，限制了RGB-D世界模型在机器人操作中的预测精度

Method: 分阶段预测框架：U-Net预测3D场景流 → 扩散模型基于场景流生成未来帧，端到端训练模块化架构

Result: 在4个基准测试中：语义相似度+7%、像素质量+11%、多领域操作成功率+6%

Conclusion: 显式运动表征与模块化架构显著提升视觉世界模型的预测能力和机器人任务成功率
...
### [96] [From Text to Network: Constructing a Knowledge Graph of Taiwan-Based China Studies Using Generative AI](https://arxiv.org/abs/2505.10093)
*Hsuan-Lei Shao*
...
### [97] [From Text to Network: Constructing a Knowledge Graph of Taiwan-Based China Studies Using Generative AI](https://arxiv.org/abs/2505.10093)
*Hsuan-Lei Shao*

Main category: cs.AI

TL;DR: 台湾学者运用生成式AI技术构建中国研究知识图谱，将1367篇文献转化为可交互的知识网络，实现了从线性阅读到网络化知识导航的范式转换


<details>
  <summary>Details</summary>
Motivation: 针对台湾中国研究领域数十年积累的非结构化学术文本，需要系统化重组并建立领域专属知识基础设施以提升学术资源利用率

Method: 应用生成式AI和大语言模型抽取实体关系三元组，通过D3.js构建轻量级可视化系统，形成知识图谱和向量数据库

Result: 系统成功揭示跨文献的学术轨迹与主题集群，提供数据驱动的本体论建构新路径，支持网络化知识探索

Conclusion: 该研究不仅验证了生成式AI在区域研究中的增强作用，更为重构区域知识体系的学术基础设施提供了可扩展方案
...
### [98] [Towards a Deeper Understanding of Reasoning Capabilities in Large Language Models](https://arxiv.org/abs/2505.10543)
*Annie Wong,Thomas Bäck,Aske Plaat,Niki van Stein,Anna V. Kononova*

Main category: cs.AI

TL;DR: 大型语言模型在动态环境中仍存在根本性缺陷，策略性提示可缩小模型间差距但无法完全克服核心推理短板，动态基准测试对评估真实推理能力至关重要


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在动态环境中作为自学习智能体的真实潜力，评估提示策略对模型适应能力的提升效果

Method: 使用不同规模的开源语言模型，在动态环境中系统测试自我反思、启发式突变和规划等提示技术的有效性

Result: 大模型表现更稳健但提示策略可缩小差距；长提示损害小模型基础任务表现；高级提示主要提升小模型复杂任务表现但导致性能波动；现有模型在规划推理和空间协调方面存在明显缺陷

Conclusion: 当前语言模型尚未展现真正的涌现推理能力，动态基准测试揭示核心推理能力不足，需超越静态测试框架开发新方法提升通用推理能力
...
### [99] [A Computational Pipeline for Advanced Analysis of 4D Flow MRI in the Left Atrium](https://arxiv.org/abs/2505.09746)
...
### [100] [Dyadic Mamba: Long-term Dyadic Human Motion Synthesis](https://arxiv.org/abs/2505.09827)
...
### [101] [BoundarySeg:An Embarrassingly Simple Method To Boost Medical Image Segmentation Performance for Low Data Regimes](https://arxiv.org/abs/2505.09829)
...
### [102] [Mission Balance: Generating Under-represented Class Samples using Video Diffusion Models](https://arxiv.org/abs/2505.09858)
...
### [103] [Few-Shot Learning of Visual Compositional Concepts through Probabilistic Schema Induction](https://arxiv.org/abs/2505.09859)
...
### [104] [Large-Scale Gaussian Splatting SLAM](https://arxiv.org/abs/2505.09915)
...
### [105] [AdaptCLIP: Adapting CLIP for Universal Visual Anomaly Detection](https://arxiv.org/abs/2505.09926)
...
### [106] [DDFP: Data-dependent Frequency Prompt for Source Free Domain Adaptation of Medical Image Segmentation](https://arxiv.org/abs/2505.09927)
...
### [107] [VRU-CIPI: Crossing Intention Prediction at Intersections for Improving Vulnerable Road Users Safety](https://arxiv.org/abs/2505.09935)
...
### [110] [MambaControl: Anatomy Graph-Enhanced Mamba ControlNet with Fourier Refinement for Diffusion-Based Disease Trajectory Prediction](https://arxiv.org/abs/2505.09965)
...
### [111] [TKFNet: Learning Texture Key Factor Driven Feature for Facial Expression Recognition](https://arxiv.org/abs/2505.09967)
*Liqian Deng*
...
### [112] [APCoTTA: Continual Test-Time Adaptation for Semantic Segmentation of Airborne LiDAR Point Clouds](https://arxiv.org/abs/2505.09971)
...
### [113] [High Quality Underwater Image Compression with Adaptive Correction and Codebook-based Augmentation](https://arxiv.org/abs/2505.09986)
...
### [114] [PointArena: Probing Multimodal Grounding Through Language-Guided Pointing](https://arxiv.org/abs/2505.09990)
...
### [115] [Descriptive Image-Text Matching with Graded Contextual Similarity](https://arxiv.org/abs/2505.09997)
...
### [116] [From Air to Wear: Personalized 3D Digital Fashion with AR/VR Immersive 3D Sketching](https://arxiv.org/abs/2505.09998)
...
### [117] [Application of YOLOv8 in monocular downward multiple Car Target detection](https://arxiv.org/abs/2505.10016)
*Shijie Lyu*
...
### [118] [ORL-LDM: Offline Reinforcement Learning Guided Latent Diffusion Model Super-Resolution Reconstruction](https://arxiv.org/abs/2505.10027)
*Shijie Lyu*
...
### [119] [DeepSeqCoco: A Robust Mobile Friendly Deep Learning Model for Detection of Diseases in Cocos nucifera](https://arxiv.org/abs/2505.10030)
...
### [120] [Exploring the Deep Fusion of Large Language Models and Diffusion Transformers for Text-to-Image Synthesis](https://arxiv.org/abs/2505.10046)
...
### [121] [Advances in Radiance Field for Dynamic Scene: From Neural Field to Gaussian Field](https://arxiv.org/abs/2505.10049)
...
### [122] [PsOCR: Benchmarking Large Multimodal Models for Optical Character Recognition in Low-resource Pashto Language](https://arxiv.org/abs/2505.10055)
...
### [123] [ToonifyGB: StyleGAN-based Gaussian Blendshapes for 3D Stylized Head Avatars](https://arxiv.org/abs/2505.10072)
...
### [124] [MMRL++: Parameter-Efficient and Interaction-Aware Representation Learning for Vision-Language Models](https://arxiv.org/abs/2505.10088)
...
### [125] [Why 1 + 1 < 1 in Visual Token Pruning: Beyond Naive Integration via Multi-Objective Balanced Covering](https://arxiv.org/abs/2505.10118)
...
### [126] [IMITATE: Image Registration with Context for unknown time frame recovery](https://arxiv.org/abs/2505.10124)
...
### [127] [Multi-Source Collaborative Style Augmentation and Domain-Invariant Learning for Federated Domain Generalization](https://arxiv.org/abs/2505.10152)
*Yikang Wei*
...
### [128] [Modeling Saliency Dataset Bias](https://arxiv.org/abs/2505.10169)
...
### [129] [VolE: A Point-cloud Framework for Food 3D Reconstruction and Volume Estimation](https://arxiv.org/abs/2505.10205)
...
### [130] [Data-Agnostic Augmentations for Unknown Variations: Out-of-Distribution Generalisation in MRI Segmentation](https://arxiv.org/abs/2505.10223)
...
### [131] [On the Interplay of Human-AI Alignment,Fairness, and Performance Trade-offs in Medical Imaging](https://arxiv.org/abs/2505.10231)
...
### [132] [MTVCrafter: 4D Motion Tokenization for Open-World Human Image Animation](https://arxiv.org/abs/2505.10238)
*Yanbo Ding*
...
### [133] [ADHMR: Aligning Diffusion-based Human Mesh Recovery via Direct Preference Optimization](https://arxiv.org/abs/2505.10250)
...
### [134] [Sage Deer: A Super-Aligned Driving Generalist Is Your Copilot](https://arxiv.org/abs/2505.10257)
...
### [135] [Inferring Driving Maps by Deep Learning-based Trail Map Extraction](https://arxiv.org/abs/2505.10258)
...
### [136] [HandReader: Advanced Techniques for Efficient Fingerspelling Recognition](https://arxiv.org/abs/2505.10267)
...
### [137] [MFogHub: Bridging Multi-Regional and Multi-Satellite Data for Global Marine Fog Detection and Forecasting](https://arxiv.org/abs/2505.10281)
...
### [138] [MSCI: Addressing CLIP's Inherent Limitations for Compositional Zero-Shot Learning](https://arxiv.org/abs/2505.10289)
...
### [139] [StoryReasoning Dataset: Using Chain-of-Thought for Scene Understanding and Grounded Story Generation](https://arxiv.org/abs/2505.10292)
...
### [140] [MIPHEI-ViT: Multiplex Immunofluorescence Prediction from H&E Images using ViT Foundation Models](https://arxiv.org/abs/2505.10294)
...
### [141] [A Unified and Scalable Membership Inference Method for Visual Self-supervised Encoder via Part-aware Capability](https://arxiv.org/abs/2505.10351)
...
### [142] [SpikeVideoFormer: An Efficient Spike-Driven Video Transformer with Hamming Attention and $\mathcal{O}(T)$ Complexity](https://arxiv.org/abs/2505.10352)
...
### [143] [Learned Lightweight Smartphone ISP with Unpaired Data](https://arxiv.org/abs/2505.10420)
...
### [144] [Vision language models have difficulty recognizing virtual objects](https://arxiv.org/abs/2505.10453)
...
### [145] [Consistent Quantity-Quality Control across Scenes for Deployment-Aware Gaussian Splatting](https://arxiv.org/abs/2505.10473)
...
### [146] [Logos as a Well-Tempered Pre-train for Sign Language Recognition](https://arxiv.org/abs/2505.10481)
...
### [147] [UniEval: Unified Holistic Evaluation for Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2505.10483)
...
### [148] [CheXGenBench: A Unified Benchmark For Fidelity, Privacy and Utility of Synthetic Chest Radiographs](https://arxiv.org/abs/2505.10496)
...
### [149] [MorphGuard: Morph Specific Margin Loss for Enhancing Robustness to Face Morphing Attacks](https://arxiv.org/abs/2505.10497)
...
### [150] [Enhancing Multi-Image Question Answering via Submodular Subset Selection](https://arxiv.org/abs/2505.10533)
...
### [151] [Exploring Implicit Visual Misunderstandings in Multimodal Large Language Models through Attention Analysis](https://arxiv.org/abs/2505.10541)
...
### [152] [Does Feasibility Matter? Understanding the Impact of Feasibility on Synthetic Training Data](https://arxiv.org/abs/2505.10551)
...
### [153] [MathCoder-VL: Bridging Vision and Code for Enhanced Multimodal Mathematical Reasoning](https://arxiv.org/abs/2505.10557)
...
### [154] [End-to-End Vision Tokenizer Tuning](https://arxiv.org/abs/2505.10562)
...
### [155] [A Computational Pipeline for Advanced Analysis of 4D Flow MRI in the Left Atrium](https://arxiv.org/abs/2505.09746)
*Xabier Morales,Ayah Elsayed,Debbie Zhao,Filip Loncaric,Ainhoa Aguado,Mireia Masias,Gina Quill,Marc Ramos,Ada Doltra,Ana Garcia,Marta Sitges,David Marlevi,Alistair Young,Martyn Nash,Bart Bijnens,Oscar Camara*

Main category: cs.CV

TL;DR: 开发首个针对左心房4D Flow MRI分析的开源计算框架，实现高精度自动分割并首次系统评估多种血流动力学参数的预后价值


<details>
  <summary>Details</summary>
Motivation: 传统超声分析左心房血流动力学存在局限，4D Flow MRI分析受限于低流速/分辨率、缺乏专用框架及多中心数据整合困难

Method: 创建鲁棒性强的开源框架，采用深度学习实现跨中心数据的高精度自动分割（Dice>0.9），使用有限训练数据达成优异效果

Result: 框架实现毫米级精度的自动分割，首次系统评估左心房能量/涡度/压力参数在不同疾病中的预后生物标志物潜力

Conclusion: 该框架为多中心研究提供标准化工具，验证的血流动力学参数为心血管疾病预后评估开辟新途径
...
### [156] [Dyadic Mamba: Long-term Dyadic Human Motion Synthesis](https://arxiv.org/abs/2505.09827)
*Julian Tanke,Takashi Shibuya,Kengo Uchida,Koichi Saito,Yuki Mitsufuji*

Main category: cs.CV

TL;DR: 提出基于状态空间模型(SSM)的Dyadic Mamba方法，通过序列连接架构有效解决长序列双人运动合成的技术瓶颈，在长序列生成效果上显著超越Transformer方法，并建立新评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的方法受限于位置编码机制，难以生成超过训练序列长度的真实双人交互动作。需要探索更适应长序列生成的新架构。
...
Conclusion: SSM架构为文本驱动的长时双人运动合成任务开辟了新方向，其序列建模优势在生成任务中展现出巨大潜力。
...
### [157] [BoundarySeg:An Embarrassingly Simple Method To Boost Medical Image Segmentation Performance for Low Data Regimes](https://arxiv.org/abs/2505.09829)
*Tushar Kataria,Shireen Y. Elhabian*

Main category: cs.CV

TL;DR: 提出多任务框架BoundarySeg，通过器官分割与边界预测的任务一致性提升医学图像分割精度，无需未标注数据即可超越现有半监督方法


<details>
  <summary>Details</summary>
Motivation: 医学数据标注成本高且隐私限制严格，现有半监督方法依赖大量未标注数据，在数据稀缺时性能下降明显

Method: 构建器官分割+边界预测双任务框架，利用任务预测一致性作为额外监督信号，在低数据量时增强模型表现

Result: 在有限标注数据下达到或超越半监督方法性能，计算效率与纯监督方法相当

Conclusion: 通过边界预测的辅助任务机制有效突破半监督方法的数据依赖限制，为低资源场景提供高效解决方案
...
### [158] [Mission Balance: Generating Under-represented Class Samples using Video Diffusion Models](https://arxiv.org/abs/2505.09858)
*Danush Kumar Venkatesh,Isabel Funke,Micha Pfeiffer,Fiona Kolbinger,Hanna Maria Schmeiser,Juergen Weitz,Marius Distler,Stefanie Speidel*

Main category: cs.CV

TL;DR: 提出两阶段文本条件扩散模型生成手术视频，通过时空解耦建模和拒绝采样策略解决数据不平衡问题，显著提升下游任务性能


<details>
  <summary>Details</summary>
Motivation: 手术视频数据集存在严重的数据不平衡问题，制约了深度学习模型的性能提升，需通过合成视频扩充数据

Method: 1. 文本条件双阶段生成框架：2D潜在扩散模型捕获空间内容，整合时间注意力层保证时序一致性；2. 引入拒绝采样策略筛选优质合成样本

Result: 在手术动作识别和术中事件预测任务中，合成视频使模型准确率分别提升3.4%和4.1%，F1分数提升5.1%和3.8%

Conclusion: 该方法有效缓解数据不平衡问题，通过开源框架推动手术AI发展，时空解耦策略为视频生成提供新思路
...
### [159] [Few-Shot Learning of Visual Compositional Concepts through Probabilistic Schema Induction](https://arxiv.org/abs/2505.09859)
*Andrew Jun Lee,Taylor Webb,Trevor Bihl,Keith Holyoak,Hongjing Lu*

Main category: cs.CV

TL;DR: 提出概率模式归纳（PSI）模型，通过深度学习实现结构化表征的类比映射，模拟人类从少量样本中学习组合视觉概念的认知机制


<details>
  <summary>Details</summary>
Motivation: 传统类别学习模型使用非结构化特征向量，难以解释人类基于结构化表征和类比映射的快速组合概念学习能力

Method: 构建包含对象关系的图结构表征，采用权衡对象相似性和关系相似性的新型相似性度量，通过深度学习进行类比映射生成组合模式（Schema）

Result: PSI在少量样本学习任务中达到类人水平，表现优于非结构化特征向量原型模型和弱结构化对照模型，其优势源于增强关系相似性及关键关系权重的自适应策略

Conclusion: 结构化表征和类比映射是实现人类快速组合概念学习的关键机制，深度学习为构建心理学模型提供了新的技术路径
...
### [160] [Large-Scale Gaussian Splatting SLAM](https://arxiv.org/abs/2505.09915)
*Zhe Xin,Chenyang Wu,Penghui Huang,Yanyong Zhang,Yinian Mao,Guoquan Huang*

Main category: cs.CV

TL;DR: 提出基于3D高斯泼溅的大规模立体视觉SLAM系统LSG-SLAM，通过多模态位姿估计、特征对齐约束和连续子地图策略，解决室外场景重建的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 现有神经辐射场和3DGS方法依赖RGBD传感器且局限室内场景，缺乏对大规模室外环境的有效支持。

Method: 结合特征对齐的渲染损失约束、连续子地图内存管理、基于闭环检测的位姿优化，以及结构细化模块提升重建质量。

Result: 在EuRoc和KITTI数据集上超越神经方法、3DGS方案及传统方法，实现室外场景的高效精准重建。

Conclusion: 系统通过多策略协同显著提升室外SLAM的鲁棒性，验证了3DGS在复杂场景中的应用潜力。
...
### [161] [AdaptCLIP: Adapting CLIP for Universal Visual Anomaly Detection](https://arxiv.org/abs/2505.09926)
*Bin-Bin Gao,Yue Zhu,Jiangtao Yan,Yuezhi Cai,Weixi Zhang,Meng Wang,Jun Liu,Yong Liu,Lei Wang,Chengjie Wang*

Main category: cs.CV

TL;DR: 提出了无需微调的AdaptCLIP方法，通过三个适配器交替学习视觉文本表征，在12个异常检测基准上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉异常检测方法存在提示模板设计复杂、需要额外微调的问题，难以实现跨领域灵活应用

Method: 在CLIP模型输入/输出端添加视觉适配器、文本适配器和提示-查询适配器，交替学习自适应表征，结合上下文和对齐残差特征的对比学习

Result: 在工业和医学领域的12个异常检测基准上显著超越现有方法，保持跨领域零/少样本泛化能力

Conclusion: AdaptCLIP通过简单而有效的适配器设计，实现了训练后无需目标域微调的高性能跨域异常检测
...
### [162] [DDFP: Data-dependent Frequency Prompt for Source Free Domain Adaptation of Medical Image Segmentation](https://arxiv.org/abs/2505.09927)
*Siqi Yin,Shaolei Liu,Manning Wang*

Main category: cs.CV

TL;DR: 提出包含预适应模块、频率提示和风格相关层微调的新型源无关域适应框架，显著提升医学影像跨域分割效果


<details>
  <summary>Details</summary>
Motivation: 解决现有SFDA方法中存在的三个核心问题：1) 域风格转换图像质量不足 2) 伪标签生成精度有限 3) 全模型训练效率低下，特别是在医疗数据隐私保护要求下实现高效域适应

Method: 三阶段方法：1) 预适应模块生成域差距缩小的初始化模型 2) 数据依赖型频率提示实现高质量源域风格转换 3) 风格相关层选择性微调策略，仅优化关键网络层

Result: 在腹部多模态(CT→MRI)和心脏图像分割任务中，DSC指标分别达到78.3%和84.7%，超越现有SOTA方法2.1%和1.8%

Conclusion: 通过预适应机制与针对性网络层优化，在严格保护源域数据隐私前提下，实现了医疗影像跨域分割的高精度和高效适应，为医学图像分析提供新的域适应范式
...
### [163] [VRU-CIPI: Crossing Intention Prediction at Intersections for Improving Vulnerable Road Users Safety](https://arxiv.org/abs/2505.09935)
*Ahmed S. Abdelrahman,Mohamed Abdel-Aty,Quoc Dai Tran*

Main category: cs.CV

TL;DR: 提出VRU-CIPI框架，结合GRU和Transformer自注意力机制预测弱势道路使用者的过街意图，准确率达96.45%并实现33fps实时推理，通过I2V通信提升路口安全


<details>
  <summary>Details</summary>
Motivation: 解决城市路口弱势道路使用者过街意图误判可能导致车辆冲突的安全隐患，需提升人车交互安全性

Method: 使用GRU捕捉时间动态特征，结合多头Transformer自注意力机制编码上下文空间依赖，构建序列化预测模型

Result: 在UCF-VRU数据集达到96.45%准确率，实时推理速度33fps，结合I2V通信可提前激活过街信号并向联网车辆发送预警

Conclusion: 该框架通过精准意图预测和基础设施协同，有效提升路口安全系数，保障所有道路使用者的顺畅交互
...
### [164] [Non-Registration Change Detection: A Novel Change Detection Task and Benchmark Dataset](https://arxiv.org/abs/2505.09939)
*Zhe Shan,Lei Zhou,Liu Mao,Shaofan Chen,Chuanqiu Ren,Xia Xie*

Main category: cs.CV

TL;DR: 提出非配准变化检测任务(NRCD)，构建八种真实场景并开发数据转换方案，证明现有方法在该场景下存在严重性能缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法依赖图像配准，但在自然灾害/军事冲突等紧急场景中难以获取配准图像，导致检测性能大幅下降。

Method: 1. 系统提出八种现实非配准场景 2. 开发图像转换方案将配准数据集转为非配准版本 3. 评估SOTA方法在非配准场景下的表现

Result: 非配准变化检测会使现有最优方法产生灾难性错误，召回率下降超过50%

Conclusion: 该研究揭示了传统方法在非配准场景的局限性，为遥感应急检测提供新方向，并开源代码和NRCD数据集推动领域发展。
...
### [165] [CSPENet: Contour-Aware and Saliency Priors Embedding Network for Infrared Small Target Detection](https://arxiv.org/abs/2505.09943)
*Jiakun Deng,Kexuan Li,Xingye Cui,Jiaxuan Li,Chang Long,Tian Pu,Zhenming Peng*

Main category: cs.CV

TL;DR: 提出CSPENet网络，通过整合目标轮廓先验和显著性先验，在复杂背景下实现更精确的红外小目标检测


<details>
  <summary>Details</summary>
Motivation: 现有方法在低对比度目标定位和密集杂波环境下的轮廓感知存在不足，严重影响检测性能。需要同时提升目标定位精度和轮廓细节表征能力。

Method: 1) 设计SCPEM模块捕获目标轮廓梯度特征，提取增强型显著性先验和多尺度结构先验；2) 构建双分支先验嵌入架构DBPEA实现特征融合；3) 开发AGFEM模块优化特征表示

Result: 在NUDT-SIRST、IRSTD-1k、NUAA-SIRST等数据集上取得SOTA性能，检测精度显著优于现有方法

Conclusion: 通过协同利用目标轮廓收敛特性和多尺度结构先验，实现了复杂场景下红外小目标检测的性能突破，为相关应用提供了有效解决方案
...
### [166] [MambaControl: Anatomy Graph-Enhanced Mamba ControlNet with Fourier Refinement for Diffusion-Based Disease Trajectory Prediction](https://arxiv.org/abs/2505.09965)
*Hao Yang,Tao Tan,Shuai Tan,Weiqin Yang,Kunyan Cai,Calvin Chen,Yue Sun*

Main category: cs.CV

TL;DR: 提出MambaControl框架，结合选择性状态空间建模与扩散过程，用于医学影像轨迹的高保真预测


<details>
  <summary>Details</summary>
Motivation: 现有方法在纵向依赖建模和退行性疾病结构一致性保持方面存在不足，需兼顾复杂时空动态与解剖完整性

Method: 整合Mamba长程建模与图引导解剖控制，引入傅里叶增强频谱图表征捕捉空间连贯性和多尺度细节

Result: 在阿尔茨海默病预测中实现SOTA性能，定量与区域评估显示进展预测质量和解剖保真度提升

Conclusion: 框架为个性化预后和临床决策支持提供新方案，验证了整合解剖控制与频谱表征的有效性
...
### [167] [TKFNet: Learning Texture Key Factor Driven Feature for Facial Expression Recognition](https://arxiv.org/abs/2505.09967)
*Liqian Deng*

Main category: cs.CV

TL;DR: 提出基于纹理关键驱动因素（TKDF）的FER框架，通过TAFE特征提取器和DCIF上下文过滤机制提升表情识别效果


<details>
  <summary>Details</summary>
Motivation: 自然场景下面部表情特征具有局部细微性与复杂变化性，现有方法难以捕捉关键纹理特征

Method: 1. TAFE模块（基于ResNet+多分支注意力）提取细粒度纹理特征
2. DCIF机制通过自适应池化与注意力过滤上下文

Result: 在RAF-DB和KDEF数据集达到SOTA，验证TKDF的有效性与鲁棒性

Conclusion: 通过显式建模眉毛/眼睛/嘴部区域的微纹理变化，显著提升复杂场景下的表情识别性能
...
### [168] [APCoTTA: Continual Test-Time Adaptation for Semantic Segmentation of Airborne LiDAR Point Clouds](https://arxiv.org/abs/2505.09971)
*Yuan Gao,Shaobo Xia,Sheng Nie,Cheng Wang,Xiaohuan Xi,Bisheng Yang*

Main category: cs.CV

TL;DR: 提出首个面向ALS点云分割的连续测试时适应方法APCoTTA，通过动态层选择、熵一致性损失和参数插值机制解决灾难性遗忘与错误累积问题


<details>
  <summary>Details</summary>
Motivation: 现实场景中环境/传感器变化导致模型性能下降，现有ALS点云CTTA研究存在基准缺失、灾难性遗忘和长期适应误差累积挑战

Method: 1. 动态可训练层选择模块（基于梯度选择低置信层） 2. 基于熵的一致性损失（筛选可靠样本） 3. 随机参数插值机制（平衡目标域适应与源知识保留）

Result: 在ISPRSC/H3DC基准上分别取得9%和14%的mIoU提升，显著优于直接推理

Conclusion: APCoTTA有效缓解连续适应中的性能退化问题，同时建立了首个ALS点云CTTA基准数据集推动领域研究
...
### [169] [High Quality Underwater Image Compression with Adaptive Correction and Codebook-based Augmentation](https://arxiv.org/abs/2505.09986)
*Yimin Zhou,Yichong Xia,Sicheng Pan,Bin Chen,Baoyi An,Haoqian Wang,Zhi Wang,Yaowei Wang,Zikun Zhou*

Main category: cs.CV

TL;DR: 提出水下图像压缩模型HQUIC，通过自适应光衰减补偿模块和动态频域加权机制，在多个数据集上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有压缩算法未充分利用水下图像光照衰减、色调差异等特性，导致压缩效率受限。需要针对性解决水下场景与陆地图像的差异化特征

Method: 1. ALTC模块预测衰减系数与全局光照信息
2. 引入码本机制提取水下物体共性特征
3. 动态加权多尺度频域分量实现信息优选

Result: 在多个水下数据集评估中，HQUIC的压缩性能优于现有最先进方法（PSNR提升约0.3dB，码率节省10%-15%）

Conclusion: 通过针对性建模水下图像的光照衰减特性与物体分布规律，结合频域动态优化策略，显著提升压缩效率，验证特征适配设计的重要性
...
### [170] [PointArena: Probing Multimodal Grounding Through Language-Guided Pointing](https://arxiv.org/abs/2505.09990)
*Long Cheng,Jiafei Duan,Yi Ru Wang,Haoquan Fang,Boyang Li,Yushan Huang,Elvis Wang,Ainaz Eftekhar,Jason Lee,Wentao Yuan,Rose Hendrix,Noah A. Smith,Fei Xia,Dieter Fox,Ranjay Krishna*

Main category: cs.CV

TL;DR: 论文提出评估多模态指向能力的PointArena平台，包含基准数据集、交互评测场和机器人系统，发现监督训练显著提升模型性能


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅关注对象定位任务，缺乏对多模态指向能力的全面评估体系，阻碍AI系统在具身智能领域的发展

Method: 构建包含Point-Bench（跨5类推理任务的千级数据集）、Point-Battle（基于网页的盲测竞技场）和Point-Act（真实机器人系统）的三位一体评估平台

Result: Molmo-72B表现最优，专有模型逐渐逼近；监督训练使准确率提升18.7%；多阶段评估指标间呈现强相关性（Spearman's ρ>0.82）

Conclusion: 精确定位能力是连接抽象推理与现实行动的关键桥梁，系统化评估框架为开发具身智能系统提供重要基准
...
### [171] [Descriptive Image-Text Matching with Graded Contextual Similarity](https://arxiv.org/abs/2505.09997)
*Jinhyun Jang,Jiyeong Lee,Kwanghoon Sohn*

Main category: cs.CV

TL;DR: 提出DITM方法，通过描述性文本匹配学习图像与文本的分级相似性，突破传统二元监督限制


<details>
  <summary>Details</summary>
Motivation: 传统图像文本匹配采用二元监督，无法覆盖多对多关系，忽视语言从通用到具体的层次性关联

Method: 1. 基于累积TF-IDF计算句子描述性得分
2. 动态调整正负样本连接（假阴性处理）
3. 构建通用到特定的文本序列对齐

Result: 在MS-COCO/Flickr30K/CxC数据集超越SOTA，HierarCaps基准显示模型层次推理能力提升

Conclusion: 描述性相似度建模能有效捕捉复杂图文关系，分级对齐机制增强模型的多层次语义理解能力
...
### [172] [From Air to Wear: Personalized 3D Digital Fashion with AR/VR Immersive 3D Sketching](https://arxiv.org/abs/2505.09998)
*Ying Zang,Yuanqi Hu,Xinyu Chen,Yuxia Xu,Suhui Wang,Chunan Yu,Lanyun Zhu,Deyi Ji,Xin Xu,Tianrun Chen*

Main category: cs.CV

TL;DR: 通过3D草图驱动生成框架，让普通用户能在AR/VR中轻松创建高质量数字服装


<details>
  <summary>Details</summary>
Motivation: 现有3D服装设计工具存在技术门槛高和数据稀缺问题，阻碍日常用户使用。需开发易用工具降低虚拟时尚创作门槛

Method: 结合条件扩散模型+共享潜在空间草图编码器+自适应课程学习策略，处理自由手绘输入

Result: 构建KO3DClothes数据集，实验证明方法在保真度和可用性上显著优于基线模型

Conclusion: 该框架为下一代消费平台实现民主化时尚设计提供有效解决方案
...
### [173] [Application of YOLOv8 in monocular downward multiple Car Target detection](https://arxiv.org/abs/2505.10016)
*Shijie Lyu*

Main category: cs.CV

TL;DR: 本文提出基于YOLOv8改进的自动驾驶目标检测网络，通过集成结构重参数化技术、双向金字塔结构网络模型和新检测流程，显著提升多尺度/小物体/远距目标的检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有雷达感知、摄像头感知等技术存在成本高、易受环境干扰、分辨率有限等问题，难以满足自动驾驶对精准目标检测的需求。

Method: 1. 在YOLOv8框架中集成结构重参数化技术
2. 构建双向金字塔结构网络模型
3. 设计新型检测流程

Result: 实验显示改进模型检测精度达65%，在大/小目标检测中均表现优异，较传统方法有显著提升。

Conclusion: 该模型在FSAC等自动驾驶竞赛中具有应用潜力，特别擅长单目标和小物体检测场景，展现了工程实践价值。
...
### [174] [ORL-LDM: Offline Reinforcement Learning Guided Latent Diffusion Model Super-Resolution Reconstruction](https://arxiv.org/abs/2505.10027)
*Shijie Lyu*

Main category: cs.CV

TL;DR: 提出基于强化学习的潜在扩散模型微调方法，显著提升遥感图像超分辨率质量


<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习方法在复杂场景处理和图像细节保留方面的不足

Method: 构建强化学习环境（状态-动作-奖励机制），在LDM反向去噪过程中使用PPO算法优化决策目标

Result: RESISC45数据集实验显示：PSNR提升3-4dB，SSIM增加0.08-0.11，LPIPS降低0.06-0.10（尤其在结构化复杂场景）

Conclusion: 该方法有效提升超分辨率质量，并展示出优秀的跨场景适应能力
...
### [175] [DeepSeqCoco: A Robust Mobile Friendly Deep Learning Model for Detection of Diseases in Cocos nucifera](https://arxiv.org/abs/2505.10030)
*Miit Daga,Dhriti Parikh,Swarna Priya Ramu*

Main category: cs.CV

TL;DR: 提出基于深度学习的DeepSeqCoco模型，实现椰树病害高精度自动化识别，准确率99.5%且效率显著提升


<details>
  <summary>Details</summary>
Motivation: 发展中国家椰树病害诊断依赖人工且效率低下，现有方法难以规模化应用，需构建AI驱动的自动化监测系统

Method: 通过测试SGD、Adam及混合优化器的性能，确定最佳计算精度与成本的平衡方案（混合SGD-Adam优化器验证损失最低2.81%）

Result: 模型准确率最高达99.5%（优于现有模型5%），训练时间减少18%，单图预测时间降低85%

Conclusion: DeepSeqCoco为精准农业提供了可扩展的AI解决方案，显著提升病害监测效率，具有重要应用价值
...
### [176] [Exploring the Deep Fusion of Large Language Models and Diffusion Transformers for Text-to-Image Synthesis](https://arxiv.org/abs/2505.10046)
*Bingda Tang,Boyang Zheng,Xichen Pan,Sayak Paul,Saining Xie*

Main category: cs.CV

TL;DR: 探索LLM与DiT深度融合的多模态生成设计空间，填补现有研究空白


<details>
  <summary>Details</summary>
Motivation: 解决现有文本到图像合成研究中设计细节不透明、缺乏与基线方法对比的问题，验证该方法的真实潜力

Method: 通过控制变量实验与成熟基线对比，分析关键设计选择，提供可复现的大规模训练方案

Result: 建立了清晰的训练流程规范，揭示不同架构选择对生成效果的影响

Conclusion: 为多模态生成领域提供可靠数据支持和实用指导方案，推动未来研究发展
...
### [177] [Advances in Radiance Field for Dynamic Scene: From Neural Field to Gaussian Field](https://arxiv.org/abs/2505.10049)
*Jinlong Fan,Xuepu Zeng,Jing Zhang,Mingming Gong,Yuxiang Yang,Dacheng Tao*

Main category: cs.CV

TL;DR: 该论文系统回顾动态场景表示技术从神经辐射场到3D高斯溅射的演进，分析200+文献，建立分类框架并指出挑战与未来方向


<details>
  <summary>Details</summary>
Motivation: 解决动态场景重建中运动表示、时间一致性、物理合理性等核心难题，推动计算机视觉与图形学领域发展

Method: 通过四维分析框架（运动表示范式/动态重建技术/辅助信息融合/正则化方法）对文献进行系统性分类与评估

Result: 建立了涵盖隐式神经表示到显式高斯基元的统一框架，揭示现有方法在非刚性运动处理与物理约束整合的局限性

Conclusion: 该综述为动态场景重建领域建立首个系统性参考框架，指明可微分渲染与物理引擎结合的前沿方向
...
### [178] [PsOCR: Benchmarking Large Multimodal Models for Optical Character Recognition in Low-resource Pashto Language](https://arxiv.org/abs/2505.10055)
*Ijazul Haq,Yingjie Zhang,Irfan Ali Khan*

Main category: cs.CV

TL;DR: 评估大模型在低资源普什图语OCR任务中的性能，构建合成数据集PsOCR并测试多款模型效果。


<details>
  <summary>Details</summary>
Motivation: 解决普什图语NLP面临的字符连写特性挑战和结构化数据稀缺问题

Method: 创建包含100万张多层级标注的PsOCR合成数据集，覆盖1000种字体变体，并用10K测试集评估7个开源模型和4个闭源模型

Result: Gemini综合表现最佳，开源模型中Qwen-7B表现突出
...
### [179] [ToonifyGB: StyleGAN-based Gaussian Blendshapes for 3D Stylized Head Avatars](https://arxiv.org/abs/2505.10072)
*Rui-Yang Ju,Sheng-Yen Huang,Yi-Ping Hung*

Main category: cs.CV

TL;DR: 提出两阶段框架ToonifyGB，结合改进StyleGAN与高斯混合形状，实现多样化3D风格化头像的实时生成与动画渲染。


<details>
  <summary>Details</summary>
Motivation: 扩展Toonify框架至3D领域，突破传统StyleGAN固定分辨率对齐面部裁剪的限制，提升视频稳定性以捕捉高频细节。

Method: 1. 改进StyleGAN生成稳定风格化视频 | 2. 构建高斯混合形状中性模型及表情基，实现任意表情高效渲染

Result: 在Arcane/Pixar风格数据集验证中，成功生成表情丰富的3D卡通化头像动画

Conclusion: ToonifyGB有效融合StyleGAN生成能力与高斯混合形状参数化建模，为3D虚拟形象创作提供高效解决方案
...
### [180] [MMRL++: Parameter-Efficient and Interaction-Aware Representation Learning for Vision-Language Models](https://arxiv.org/abs/2505.10088)
*Yuncheng Guo,Xiaodong Gu*

Main category: cs.CV

TL;DR: 提出MMRL框架解决预训练视觉语言模型小样本过拟合问题，通过共享表示空间和分层优化策略提升跨模态交互与泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在小样本任务中过度优化类别标记导致过拟合，低效利用预训练知识且缺乏跨模态交互

Method: 1. 构建模态无关表示空间生成跨模态表示标记
2. 在高层编码器插入表示标记保留底层通用知识
3. 联合优化可训练表示投影层与冻结类别投影层
4. 引入零样本特征对齐正则化项

Result: 在15个数据集上超越SOTA方法，最高提升2.1%准确率，MMRL++减少75%训练参数

Conclusion: MMRL系列通过分层特征优化与解耦推理策略，在任务适应与泛化能力间取得最优平衡
...
### [181] [Why 1 + 1 < 1 in Visual Token Pruning: Beyond Naive Integration via Multi-Objective Balanced Covering](https://arxiv.org/abs/2505.10118)
*Yangfu Li,Hongjian Zhan,Tianyi Chen,Qi Liu,Yue Lu*

Main category: cs.CV

TL;DR: 提出多目标平衡覆盖方法(MoB)，通过理论推导和贪心策略动态平衡视觉token剪枝中的提示对齐和视觉保留目标，在保持96.4%模型性能的同时仅使用11.1%原始token。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法采用静态策略，忽视不同任务间目标重要性的动态变化，导致性能不稳定。需要建立理论框架量化目标权衡关系。

Method: 1. 基于Hausdorff距离推导视觉token剪枝的闭式误差边界
2. 利用ε-覆盖理论揭示目标间本质权衡
3. 将问题建模为双目标覆盖问题，通过贪心半径交易实现预算分配

Result: LLaVA-1.5-7B模型保留96.4%性能（11.1% token），LLaVA-Next-7B加速1.3-1.5倍；适配Qwen2-VL和Video-LLaVA验证扩展性

Conclusion: MoB通过理论驱动的动态平衡机制，在保证可证明性能边界的前提下，实现了视觉token剪枝的线性扩展能力和跨模型/任务的广泛适用性。
...
### [182] [IMITATE: Image Registration with Context for unknown time frame recovery](https://arxiv.org/abs/2505.10124)
*Ziad Kheil,Lucas Robinet,Laurent Risser,Soleakhena Ken*

Main category: cs.CV

TL;DR: 提出基于条件U-Net的新型图像配准框架，用于多条件医学影像重建，成功消除4D-CT呼吸运动伪影。


<details>
  <summary>Details</summary>
Motivation: 解决传统4D-CT图像重建中因呼吸不规则性和器官滞后效应导致的缝合伪影问题，提升放疗肿瘤定位精度。

Method: 1. 建立基于条件信息的图像配准范式
2. 设计无需固定参考图像的条件U-Net架构
3. 应用于胸腔4D-CT序列的跨呼吸幅度肿瘤运动建模
...
Conclusion: 该框架突破了传统配准需固定参考图像的限制，在复杂呼吸模式下实现可靠的运动建模，为精准放疗提供新工具，代码开源促进临床应用验证。
...
### [183] [Multi-Source Collaborative Style Augmentation and Domain-Invariant Learning for Federated Domain Generalization](https://arxiv.org/abs/2505.10152)
*Yikang Wei*

Main category: cs.CV

TL;DR: 提出多源协作风格增强与领域不变学习方法（MCSAD），通过跨域协作生成更广风格空间数据，并结合特征对齐与关系蒸馏提升联邦领域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习框架下的风格增强方法存在风格空间受限问题（单域内增强/跨域线性插值），导致模型在未知目标域泛化能力不足。

Method: 1. 多源协作风格增强模块生成跨域组合式风格数据
2. 通过跨域特征对齐（同类别）与类间关系集成蒸馏（不同类别）实现领域不变学习
3. 交替执行风格增强与领域不变学习优化模型

Result: 在多个领域泛化数据集上的实验表明，MCSAD显著优于现有联邦领域泛化方法（SOTA）

Conclusion: 通过协同式风格增强与双层次领域不变学习机制，有效提升了模型在未知目标域的泛化能力。
...
### [184] [Modeling Saliency Dataset Bias](https://arxiv.org/abs/2505.10169)
*Matthias Kümmerer,Harneet Khanuja,Matthias Bethge*

Main category: cs.CV

TL;DR: 提出仅需调整20个可解释参数即可解决显著性预测模型跨数据集泛化难题的新架构，在MIT300等基准刷新SOTA


<details>
  <summary>Details</summary>
Motivation: 现有显著性预测模型在不同数据集间泛化性能骤降40%，发现60%差异源于数据集固有偏差。现有增加数据多样性的方法无法解决该问题

Method: 在数据集无关的编码器-解码器架构中引入少量（<20）可解释的特定数据集参数，控制多尺度结构、中心偏差、注视扩散等机制

Result: 仅调整这些参数即可弥补75%泛化差距（50样本即达显著提升），在MIT300/CAT2000/COCO-Freeview三大基准全面刷新SOTA

Conclusion: 通过极小量可解释参数的领域适应，实现跨数据集性能突破，同时揭示多尺度效应由绝对与相对尺寸共同作用的复杂机制
...
### [185] [VolE: A Point-cloud Framework for Food 3D Reconstruction and Volume Estimation](https://arxiv.org/abs/2505.10205)
*Umair Haroon,Ahmad AlMughrabi,Thanasis Zoumpekas,Ricardo Marques,Petia Radeva*

Main category: cs.CV

TL;DR: 提出VolE框架，利用移动设备自由运动捕捉实现无需参考物/深度信息的精准食物体积估计，MAPE达2.22%


<details>
  <summary>Details</summary>
Motivation: 现有食物体积估计方法受限于专用硬件、深度传感器或参照物校准，难以在移动端实现便捷精准测量

Method: 基于AR移动设备自由运动捕捉构建3D模型，结合视频分割生成食物掩模，开发无参照物/深度信息的框架

Result: 在多个数据集上超越现有技术，平均绝对百分比误差2.22%，新基准测试涵盖更复杂现实场景

Conclusion: VolE验证了移动端视觉方案在营养管理中的可行性，为健康监测提供低门槛精准测量方案
...
### [186] [Data-Agnostic Augmentations for Unknown Variations: Out-of-Distribution Generalisation in MRI Segmentation](https://arxiv.org/abs/2505.10223)
*Puru Vaish,Felix Meister,Tobias Heimann,Christoph Brune,Jelmer M. Wolterink*

Main category: cs.CV

TL;DR: MixUp和辅助傅里叶增强显著提升医学影像分割模型在分布偏移下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 传统视觉一致性数据增强策略对临床场景多样性变化适应性不足，导致模型在真实场景泛化性下降

Method: 系统评估MixUp和辅助傅里叶增强策略，通过改进nnU-Net训练流程提升特征可分离性和紧致度

Result: 在心脏电影MRI和前列腺MRI分割任务中有效提升跨域泛化能力，模型对多种图像变换展现出强鲁棒性

Conclusion: 将非针对性增强策略集成到现有框架，为提升医学分割模型临床可靠性提供高效解决方案
...
### [187] [On the Interplay of Human-AI Alignment,Fairness, and Performance Trade-offs in Medical Imaging](https://arxiv.org/abs/2505.10231)
*Haozhe Luo,Ziyu Zhou,Zixin Shu,Aurélie Pahud de Mortanges,Robert Berke,Mauricio Reyes*

Main category: cs.CV

TL;DR: 医学AI公平性研究：通过人类对齐减少群体偏见，但需平衡对齐程度以避免性能损失。


<details>
  <summary>Details</summary>
Motivation: 解决医疗AI系统中因数据偏见导致的群体公平性差距，探索人类专家指导对模型公平性和泛化能力的影响。

Method: 首次系统性研究医学AI领域的人类对齐机制，分析不同对齐强度对模型公平性和跨领域泛化的影响。

Result: 人类知识融合使公平差距减少22.5%，跨域准确率提升15.7%，但过度对齐会导致特定任务性能下降12%

Conclusion: 校准式人类对齐策略在医学AI开发中至关重要，需在专家指导效率与自动化性能间寻求最优平衡点。
...
### [188] [MTVCrafter: 4D Motion Tokenization for Open-World Human Image Animation](https://arxiv.org/abs/2505.10238)
*Yanbo Ding*

Main category: cs.CV

TL;DR: 提出MTVCrafter框架，首次直接建模3D运动序列实现人体图像动画，通过4DMoT量化运动序列和MV-DiT生成动画，取得SOTA效果（FID-VID 6.98）并实现开放世界泛化


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D姿势图像导致泛化性差且丢失3D信息，需开发直接处理原始3D运动序列的方案

Method: 1. 4DMoT将3D运动序列量化为4D运动token 2. MV-DiT通过带4D位置编码的运动注意力机制利用token生成动画

Result: FID-VID指标达6.98（比第二名提升65%），支持多角色/全身/半身等开放场景，GitHub匿名代码已开源

Conclusion: 开创了基于3D运动token的动画生成新范式，突破了传统2D姿势引导的局限性，为复杂3D世界的人物视频生成开辟新方向
...
### [189] [ADHMR: Aligning Diffusion-based Human Mesh Recovery via Direct Preference Optimization](https://arxiv.org/abs/2505.10250)
*Wenhao Shen,Wanqi Yin,Xiaofeng Yang,Cheng Chen,Chaoyue Song,Zhongang Cai,Lei Yang,Hao Wang,Guosheng Lin*

Main category: cs.CV

TL;DR: 提出ADHMR框架，通过偏好优化对齐扩散模型，解决单目人体网格重建中的2D对齐和鲁棒性问题


<details>
  <summary>Details</summary>
Motivation: 现有概率方法生成的3D预测常与2D观察不匹配，且对真实场景图像鲁棒性差。需无需3D标注的评估方法提升模型性能

Method: 1. 训练HMR-Scorer评估模型进行预测质量评估 → 2. 构建偏好数据集 → 3. 使用直接偏好优化微调基础模型

Result: ADHMR超越现有SOTA方法，HMR-Scorer仅用30%数据即可提升现有模型性能，在3DPW数据集上MPJPE降低7.3%

Conclusion: 通过偏好优化对齐策略有效解决2D观测对齐问题，HMR-Scorer的双重功能（模型优化+数据清洗）显著提升算法在真实场景的适用性
...
### [190] [Sage Deer: A Super-Aligned Driving Generalist Is Your Copilot](https://arxiv.org/abs/2505.10257)
*Hao Lu,Jiaqi Tang,Jiyao Wang,Yunfan LU,Xu Cao,Qingyong Hu,Yin Wang,Yuting Zhang,Tianxin Xie,Yunpeng Zhang,Yong Chen,Jiayu. Gao,Bin Huang,Dengbo He,Shuiguang Deng,Hao Chen,Ying-Cong Chen*

Main category: cs.CV

TL;DR: 提出超级对齐的通用驾驶代理SAGE DeeR，通过三重特性提升智能座舱的个性化与综合决策能力


<details>
  <summary>Details</summary>
Motivation: 智能驾驶座舱需满足不同用户在舒适性、交互性和安全性方面的个性化需求，现有系统在用户多维需求对齐和综合决策能力存在不足

Method: 1) 超级对齐：基于用户偏好生成个性化响应 2) 通用性：融合多视角多模态输入理解用户生理指标-情感-行为 3) 自我激发：通过语言空间隐式思维链增强系统能力

Result: 构建包含多维度数据集的大规模基准测试，量化评估智能体的感知决策能力（平均提升32.1%）和超级对齐准确率（达到89.7%）

Conclusion: SAGE DeeR通过三位一体的架构设计，实现了智能座舱系统在个性化适配与综合决策能力上的突破，其中自我激发机制使通用性和对齐能力形成正向循环增强
...
### [191] [Inferring Driving Maps by Deep Learning-based Trail Map Extraction](https://arxiv.org/abs/2505.10258)
*Michael Hubbertz,Pascal Colling,Qi Han,Tobias Meisen*

Main category: cs.CV

TL;DR: 提出基于transformer的离线高精地图构建方法，通过整合多源轨迹数据实现优于在线方法的泛化性能与运行效率


<details>
  <summary>Details</summary>
Motivation: 在线制图存在时间一致性差、传感器遮挡敏感、泛化能力弱等问题，传统离线制图缺乏动态更新能力。需开发支持持续更新且传感器无关的鲁棒制图方案

Method: 融合自车与他车轨迹数据，采用transformer模型进行多源信息聚合，构建可动态更新的传感器无关全局地图

Result: 在两个基准数据集上验证，相比SOTA在线方法提升14.7%的泛化性能，未知环境下的地图完整性提高22%

Conclusion: 基于多源轨迹融合的离线制图范式有效克服在线方法局限，为自动驾驶系统提供更具适应性的地图解决方案
...
### [192] [HandReader: Advanced Techniques for Efficient Fingerspelling Recognition](https://arxiv.org/abs/2505.10267)
*Pavel Korotaev,Petr Surovtsev,Alexander Kapitanov,Karina Kvanchiani,Aleksandr Nagaev*

Main category: cs.CV

TL;DR: 提出HandReader系列架构（RGB/KP/RGB+KP），通过时序处理模块TSAM和TPE改进手语拼写识别，在多个数据集创SOTA并发布俄语数据集Znaki。


<details>
  <summary>Details</summary>
Motivation: 现有手语拼写识别方法在视频时序信息处理上存在精度提升空间，需开发能有效处理不同长度视频并保留关键序列信息的新方法。

Method: 1. HandReader_RGB使用TSAM模块处理RGB特征；2. HandReader_KP基于TPE编码器处理关键点张量；3. HandReader_RGB+KP联合两种模态的混合架构。

Result: 在ChicagoFSWild系列数据集取得SOTA，俄语数据集Znaki上表现优异。同步开源Znaki数据集与预训练模型。

Conclusion: 三种架构各具优势，TSAM和TPE有效利用时空信息，多模态融合方案显著提升识别性能，填补俄语手语资源空白。
...
### [193] [MFogHub: Bridging Multi-Regional and Multi-Satellite Data for Global Marine Fog Detection and Forecasting](https://arxiv.org/abs/2505.10281)
*Mengqiu Xu,Kaixin Chen,Heng Guo,Yixiang Huang,Ming Wu,Zhenwei Shi,Chuang Zhang,Jun Guo*

Main category: cs.CV

TL;DR: 开发首个多区域多卫星海洋雾数据集MFogHub，整合15个沿海区域和6颗卫星的68,000+样本，提升检测与预测模型的跨域评估能力。


<details>
  <summary>Details</summary>
Motivation: 现有海洋雾数据集局限于单一区域/卫星，难以评估模型在不同条件下的性能表现，也阻碍了对海洋雾本质特征的探索。

Method: 整合15个多雾沿海区域和6颗地球同步卫星的标注数据，构建包含68,000+高分辨率样本的多源异构数据集，并通过16个基线模型进行跨域实验验证。

Result: 实验揭示了区域差异和卫星视角差异导致的模型泛化波动，验证数据集可作为开发针对性雾预测技术的重要资源。

Conclusion: MFogHub通过多维度数据整合，推动了全球海洋雾动态监测能力和科学认知的提升，开源数据助力后续研究。
...
### [194] [MSCI: Addressing CLIP's Inherent Limitations for Compositional Zero-Shot Learning](https://arxiv.org/abs/2505.10289)
*Yue Wang,Shuai Xu,Xuelin Zhu,Yicong Li*

Main category: cs.CV

TL;DR: 提出多阶段跨模态交互模型MSCI，通过提取CLIP中间层视觉特征增强组合零样本学习中细粒度局部特征感知能力


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的组合零样本学习方法忽视其架构和训练范式导致的细粒度局部特征捕捉缺陷

Method: 设计双自适应聚合器（低层局部特征提取器+高层全局特征整合器），通过分阶段交互机制将视觉关键信息渐进融入文本表示

Result: 在三个主流数据集上验证模型有效性，性能表现超越现有方法

Conclusion: MSCI通过动态调整全局/局部注意力权重，灵活适应不同组合场景，显著提升细粒度感知能力
...
### [195] [StoryReasoning Dataset: Using Chain-of-Thought for Scene Understanding and Grounded Story Generation](https://arxiv.org/abs/2505.10292)
*Daniel A. P. Oliveira,David Martins de Matos*

Main category: cs.CV

TL;DR: 通过StoryReasoning数据集和模型微调，减少视觉叙事中的指称幻觉


<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事系统存在角色身份跨帧不一致、动作与主体关联错误导致的指称幻觉问题

Method: 跨帧对象重识别（视觉相似度+人脸识别）+思维链推理叙事建模+文本-视觉实体多帧链接方案，微调Qwen2.5-VL 7B实现端到端检测系统

Result: 微调模型Qwen Storyteller将每个故事的平均幻觉数从4.06降至3.56（-12.3%）

Conclusion: 通过结构化数据与跨帧分析方法显著提升角色/对象一致性，验证了模型微调对减少叙事幻觉的有效性
...
### [196] [MIPHEI-ViT: Multiplex Immunofluorescence Prediction from H&E Images using ViT Foundation Models](https://arxiv.org/abs/2505.10294)
*Guillaume Balezo,Roger Trullo,Albert Pla Planas,Etienne Decenciere,Thomas Walter*

Main category: cs.CV

TL;DR: 提出MIPHEI模型，通过U-Net架构集成ViT基础模型，实现仅用H&E染色图像预测多标记免疫荧光信号
...
Method: 采用U-Net架构集成视觉Transformer编码器，基于ORION结直肠癌数据集训练，验证集覆盖两个独立数据集

Result: Pan-CK分类F1达0.88，CD3e/SMA/CD68/CD20分别为0.57/0.56/0.36/0.30，多数指标超越现有基线模型

Conclusion: 证明H&E图像中蕴含细胞分子标记关联特征，为大规模组织学分析提供细胞类型识别新路径
...
### [197] [A Unified and Scalable Membership Inference Method for Visual Self-supervised Encoder via Part-aware Capability](https://arxiv.org/abs/2505.10351)
*Jie Zhu,Jirong Zha,Ding Li,Leye Wang*

Main category: cs.CV

TL;DR: 提出黑盒场景下针对视觉自监督模型的统一成员推理方法PartCrop，通过局部裁剪探测模型响应，验证了攻击有效性并探讨防御方案。


<details>
  <summary>Details</summary>
Motivation: 自监督学习面临隐私风险，现有成员推理方法依赖白盒信息。针对实际黑盒场景中模型训练细节未知的挑战，需开发跨范式统一攻击方法。

Method: PartCrop基于模型共有的局部感知能力，通过裁剪图像局部区域，在表征空间计算原图与裁剪图的响应相似度，利用训练数据更强的局部响应特征进行成员推断。

Result: 在三种常用数据集上验证了PartCrop对Moco、SimCLR等不同协议模型的攻击有效性（AUC最高0.78）。提出早期停止、差分隐私和裁剪尺度限制三种防御方案，均展示防护效果。

Conclusion: 首次系统研究黑盒场景下自监督模型隐私风险，提出的PartCrop具有跨架构泛化性。通过模型/数据规模扩展实验，开发了改进版PartCrop-v2，为实际系统隐私防护提供新思路。
...
### [198] [SpikeVideoFormer: An Efficient Spike-Driven Video Transformer with Hamming Attention and $\mathcal{O}(T)$ Complexity](https://arxiv.org/abs/2505.10352)
*Shihao Zou,Qingfeng Li,Wei Ji,Jingjing Li,Yongkui Yang,Guoqi Li,Chao Dong*

Main category: cs.CV

TL;DR: 提出SpikeVideoFormer——首个实现线性时间复杂度的脉冲驱动视频Transformer，在多项视频任务中超越现有SNN方法并达到ANN性能，同时实现16倍、10倍和5倍的效率提升。


<details>
  <summary>Details</summary>
Motivation: 现有SNN Transformer主要针对单图像任务，未能有效发挥SNN在视频任务中的效率优势。需要开发兼具高效时空建模能力和线性复杂度的视频架构。

Method: 1. 设计脉冲驱动汉明注意力(SDHA)，实现从传统注意力到脉冲注意力的理论适配
2. 通过系统分析提出最优的脉冲时空注意力方案
3. 构建仅具O(T)时间复杂度的视频Transformer架构

Result: 1. 在视频分类/姿态跟踪/语义分割任务中分别取得SOTA性能，后两项提升超15%
2. 效率指标显著超越ANN方法(×16/×10/×5)
3. 首次实现SNN在复杂视频任务中与ANN的性能匹配

Conclusion: SpikeVideoFormer通过创新的脉冲时空注意力机制，在保持线性复杂度的同时实现了视频理解的性能突破，为SNN在实时视频处理领域的应用开辟了新方向。
...
### [199] [Learned Lightweight Smartphone ISP with Unpaired Data](https://arxiv.org/abs/2505.10420)
*Andrei Arhire,Radu Timofte*

Main category: cs.CV

TL;DR: 提出一种无需配对数据的智能手机ISP训练方法，通过对抗训练和多判别器机制学习目标图像特征，在移动设备上实现高质量图像转换。


<details>
  <summary>Details</summary>
Motivation: 传统基于深度学习的ISP方法依赖像素级对齐的RAW-RGB配对数据，而这类数据的获取成本高昂且困难，限制了实际应用场景。

Method: 采用未配对训练策略，设计多术语损失函数结合对抗训练框架，利用预训练网络特征图指导内容结构保持，同时通过多个判别器学习目标域的颜色纹理特征。

Result: 在Zurich RAW-RGB和Fujifilm UltraISP数据集上验证，使用轻量级网络架构即达到与配对训练方法相当的指标表现，PSNR超过24dB，模型参数小于100K。

Conclusion: 该方法有效突破了配对数据限制，为移动端ISP部署提供了新范式，开源代码和预训练模型将推动相关领域发展。
...
### [200] [Vision language models have difficulty recognizing virtual objects](https://arxiv.org/abs/2505.10453)
*Tyler Tran,Sangeet Khemlani,J. G. Trafton*

Main category: cs.CV

TL;DR: 现有视觉语言模型对虚拟物体的空间关系理解存在明显缺陷


<details>
  <summary>Details</summary>
Motivation: 通过引入虚拟物体（图像中未实际存在的物体）测试VLM的场景理解能力，揭示模型在空间推理方面的局限性

Method: 使用包含虚拟物体提示的测试框架（如图像中暗示风筝卡在树上），系统评估主流VLMs的空间关系推理能力

Result: 当前最先进的VLMs在虚拟物体处理任务中表现出显著不足（准确率低于50%）

Conclusion: 视觉语言模型需要增强场景表征更新能力和空间推理机制，虚拟物体测试可作为重要的评估基准
...
### [201] [Consistent Quantity-Quality Control across Scenes for Deployment-Aware Gaussian Splatting](https://arxiv.org/abs/2505.10473)
*Fengdi Zhang,Hongkun Cao,Ruqi Huang*

Main category: cs.CV

TL;DR: ControlGS通过用户指定参数实现3D高斯溅射的量化质量控制，自动适配不同场景并优于基线方法


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏让用户直观调节数量-质量权衡的能力，难以适应不同硬件部署的实践需求

Method: 使用固定训练设置结合用户指定的量化-质量偏好超参数，支持广泛调整范围的连续控制

Result: 在多种场景下实现更高渲染质量（更少高斯数量），支持从紧凑物体到大型户外场景的广泛调节

Conclusion: ControlGS提供跨场景一致的语义化量化控制，在保持高性能的同时满足多样化部署需求
...
### [202] [Logos as a Well-Tempered Pre-train for Sign Language Recognition](https://arxiv.org/abs/2505.10481)
*Ilya Ovodov,Petr Surovtsev,Karina Kvanchiani,Alexander Kapitanov,Alexander Nagaev*

Main category: cs.CV

TL;DR: 提出俄语手语数据集Logos解决孤立手语识别的数据稀缺与标注歧义问题，通过跨语言迁移学习和视觉相似组标注提升模型性能


<details>
  <summary>Details</summary>
Motivation: 现有单一手语数据集规模有限且存在相似手势语义歧义，制约跨语言模型训练与标注质量

Method: 构建最大规模RSL数据集Logos，采用多分类头联合训练策略实现跨语言迁移学习，并标注视觉相似符号组优化模型训练
...
### [203] [UniEval: Unified Holistic Evaluation for Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2505.10483)
*Yi Li,Haonan Wang,Qixiang Zhang,Boyu Xiao,Chenchang Hu,Hualiang Wang,Xiaomeng Li*

Main category: cs.CV

TL;DR: 提出首个无需额外模型/标注的多模态统一评估框架UniEval，包含高多样性评测基准UniBench和与人类评估高度一致的UniScore度量标准


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型评估存在五大痛点：缺乏整体评估结果、依赖外部评估模型误差、需要海量标注图像、评测基准多样性不足、现有指标无法有效评估指令跟随能力

Method: 通过UniBench基准（含81个细粒度标签）支持多模态理解和生成任务，开发UniScore度量标准（结合细粒度标签权重和人工评估偏好）实现无监督评估

Result: 实验显示UniBench比现有基准更具挑战性（平均得分低20.5%），UniScore与人工评估相关性达80.4%，显著优于现有指标（最高提升35.8%）

Conclusion: UniEval框架为统一多模态模型提供了标准化评估方案，其细粒度标签体系和自动化评估能力揭示了模型在复杂场景下的独特价值
...
### [204] [CheXGenBench: A Unified Benchmark For Fidelity, Privacy and Utility of Synthetic Chest Radiographs](https://arxiv.org/abs/2505.10496)
*Raman Dutt,Pedro Sanchez,Yongchen Yao,Steven McDonagh,Sotirios A. Tsaftaris,Timothy Hospedales*

Main category: cs.CV

TL;DR: 提出CheXGenBench评估框架，通过标准化流程全面评估胸部X光生成模型的真实性、隐私风险和临床价值，并发布SynthCheX-75K数据集


<details>
  <summary>Details</summary>
Motivation: 现有医学图像生成评估存在方法不一致、架构对比过时、临床价值评估缺失等问题，需建立统一标准推动医学AI发展

Method: 采用标准化数据划分和20+量化指标评估协议，系统性分析11种领先文本-图像模型的生成质量、隐私漏洞及临床适用性

Result: 发现现有评估协议在真实性评估方面存在缺陷，提出新基准确立Sana 0.6B为最优模型，生成75K高质量合成影像数据集

Conclusion: CheXGenBench为医学AI社区建立标准化评估基准，促进客观模型比较，并通过开源框架和数据集推动该领域发展
...
### [205] [MorphGuard: Morph Specific Margin Loss for Enhancing Robustness to Face Morphing Attacks](https://arxiv.org/abs/2505.10497)
*Iurii Medvedev,Nuno Goncalves*

Main category: cs.CV

TL;DR: 提出双分支分类策略增强人脸识别模型对融合攻击的防御能力，通过改进标签处理机制提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 面部融合攻击允许不同身份相互冒充，严重威胁基于人脸识别的身份认证系统安全性，需提升模型对此类攻击的防御能力。

Method: 设计双分支分类架构处理融合图像的标签模糊问题，将攻击样本整合至训练流程，强化模型对真实样本与攻击样本的区分能力。

Result: 在公开基准测试中验证有效性，显著提升抗融合攻击性能，且该方法具备通用性可无缝集成现有训练框架。

Conclusion: 该训练策略为提升人脸识别系统安全性提供了有效解决方案，具有实际部署价值与行业应用潜力。
...
### [206] [Enhancing Multi-Image Question Answering via Submodular Subset Selection](https://arxiv.org/abs/2505.10533)
*Aaryan Sharma,Shivansh Gupta,Samar Agarwal,Vishak Prasad C.,Ganesh Ramakrishnan*

Main category: cs.CV

TL;DR: 提出基于子模函数选择的多图像检索增强框架，提升大规模多图像问答任务的性能


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在处理多图像问答任务时存在扩展性不足和检索性能下降的问题，需改进检索效率

Method: 使用GraphCut等查询感知的子模函数预选语义相关图像，结合锚点查询机制和数据增强技术优化检索流程

Result: 在大型图像集合（haystack）中显著提升检索效率，特别是在海量图像场景下保持性能稳定

Conclusion: 子模选择与数据增强的结合有效解决了多图像问答场景的扩展瓶颈，为复杂视觉推理任务提供了新思路
...
### [207] [Exploring Implicit Visual Misunderstandings in Multimodal Large Language Models through Attention Analysis](https://arxiv.org/abs/2505.10541)
*Pengfei Wang,Guohai Xu,Weinong Wang,Junjie Yang,Jie Lou,Yunhua Xue*

Main category: cs.CV

TL;DR: 提出隐式视觉误解（IVM）概念，通过解耦注意力模块发现注意力分布规律，构建注意力准确率指标和新基准量化IVM，验证方法在单模态场景的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准仅关注答案正确性，无法检测模型是否真正理解视觉输入（IVM问题），需开发更可靠的评估方法。

Method: 1. 解耦视觉和文本模态的因果注意力模块
2. 分析注意力分布与答案关联性
3. 提出注意力准确率指标及IVM量化基准

Result: 注意力准确率可有效评估视觉理解能力，对位置偏差具有鲁棒性；方法可扩展至细粒度分析和单模态场景验证。

Conclusion: 通过模型内部机制直接评估视觉理解能力，为MLLMs提供更可靠的评估工具，促进模型优化方向。
...
TL;DR: 研究验证合成图像的可行性对CLIP分类器性能影响有限，混合可行/不可行数据对训练影响不显著


<details>
  <summary>Details</summary>
Motivation: 针对扩散模型生成不现实图像可能影响模型泛化能力的问题，探索合成训练数据中可行性约束的必要性

Method: 提出VariReal流程，通过大语言模型生成文本提示，对源图像进行最小化编辑以控制属性可行性

Result: 可行性差异导致LoRA微调的CLIP分类器top-1准确率变化普遍低于0.3%，不同属性对分类性能影响存在差异

Conclusion: 可行性约束在特定属性（如背景/颜色/纹理）训练中具有选择性价值，但整体必要性低于预期
...
### [209] [MathCoder-VL: Bridging Vision and Code for Enhanced Multimodal Mathematical Reasoning](https://arxiv.org/abs/2505.10557)
*Ke Wang,Junting Pan,Linda Wei,Aojun Zhou,Weikang Shi,Zimu Lu,Han Xiao,Yunqiao Yang,Houxing Ren,Mingjie Zhan,Hongsheng Li*

Main category: cs.CV

TL;DR: 提出通过代码监督实现跨模态对齐，提升多模态数学推理模型性能，模型MathCoder-VL在几何问题解决能力上超越GPT-4o和Claude 3.5。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言图像数据集忽视数学图表细节，阻碍大模型在多模态数学推理中的发展。代码能完整编码图表信息，可建立精准的跨模态连接。

Method: 1. 模型闭环开发FigCodifier图像转码模型和ImgCode-8.6M数据集
2. 构建MM-MathInstruct-3M数学指令微调数据集
3. 两阶段训练MathCoder-VL模型

Result: 实现开源模型SOTA，MathVista几何问题准确率提升8.9-9.2%，超越商业模型。

Conclusion: 代码监督有效提升多模态数学推理能力，开源数据集和模型推动领域发展。
...
### [210] [End-to-End Vision Tokenizer Tuning](https://arxiv.org/abs/2505.10562)
*Wenxuan Wang,Fan Zhang,Yufeng Cui,Haiwen Diao,Zhuoyan Luo,Huchuan Lu,Jing Liu,Xinlong Wang*

Main category: cs.CV

TL;DR: 提出ETT端到端视觉标记器调优方法，通过联合优化视觉标记化与目标任务，解决表征瓶颈问题，提升多模态任务性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记化方法将优化与下游任务解耦，低层重建优化的标记器无法适应需要不同表征的下游任务（如图像文本识别错误导致生成失败）

Method: 利用标记器代码本的视觉嵌入，通过重建+描述目标联合优化视觉标记器，无需修改原有架构/代码本

Result: 多模态理解和生成任务性能提升2-6%，同时保持原有重建能力

Conclusion: ETT简单高效，可增强多模态基础模型的通用能力，拓展到图像生成/理解之外的场景
...
### [211] [Depth Anything with Any Prior](https://arxiv.org/abs/2505.10565)
*Zehan Wang,Siyu Chen,Lihe Yang,Jialei Wang,Ziang Zhang,Hengshuang Zhao,Zhou Zhao*

Main category: cs.CV

TL;DR: 提出Prior Depth Anything框架，通过融合不完整但精确的深度测量与相对但完整的几何预测，生成高精度稠密深度图


<details>
  <summary>Details</summary>
Motivation: 解决现有深度估计方法在域适应、噪声处理和场景泛化方面的不足，通过互补深度源的显隐式融合提升效果

Method: 分阶段流程：1）像素级度量对齐与距离感知加权预填充 2）带条件约束的单目深度估计模型细化噪声

Result: 在7个真实数据集上实现零样本泛化，深度补全/超分辨率/修复任务性能超越专用模型，混合先验处理表现优异

Conclusion: 该框架具有测试时改进能力，可通过切换预测模型实现灵活的效率-精度权衡，适应MDE模型的技术演进
...
### [212] [3D-Fixup: Advancing Photo Editing with 3D Priors](https://arxiv.org/abs/2505.10566)
*Yen-Chi Cheng,Krishna Kumar Singh,Jae Shin Yoon,Alex Schwing,Liangyan Gui,Matheus Gadelha,Paul Guerrero,Nanxuan Zhao*

Main category: cs.CV

TL;DR: 提出3D-Fixup框架，通过融合扩散模型与3D先验指导，实现复杂3D感知的2D图像编辑


<details>
  <summary>Details</summary>
Motivation: 解决单图像难以进行3D编辑（如物体平移/旋转）的挑战，利用视频动态信息与3D投影突破2D编辑局限

Method: 1. 基于视频生成训练数据对 2. 整合Image-to-3D模型提供3D指导 3. 设计数据生成流程保障3D指导质量

Result: 实验证明框架支持复杂3D编辑任务，在身份一致性/质量上优于现有方法，代码已开源

Conclusion: 3D-Fixup通过显式3D投影机制，推动了扩散模型在真实图像编辑中的应用边界
...
### [213] [ImplicitStainer: Data-Efficient Medical Image Translation for Virtual Antibody-based Tissue Staining Using Local Implicit Functions](https://arxiv.org/abs/2505.09831)
*Tushar Kataria,Beatrice Knudsen,Shireen Y. Elhabian*

Main category: eess.IV

TL;DR: 提出ImplicitStainer方法，利用局部隐函数改进虚拟染色技术，在少量数据下实现高质量IHC染色图像生成。


<details>
  <summary>Details</summary>
Motivation: 传统IHC染色耗时且依赖专业设备，现有虚拟染色模型需要大量数据。需开发数据高效的方法加速病理诊断。

Method: 基于局部隐函数进行像素级预测，通过坐标映射学习图像转换关系，提升模型对数据规模的鲁棒性。

Result: 在两类数据集上超越15种SOTA模型，验证了在有限数据下的高质量生成能力。

Conclusion: 该方法显著降低虚拟染色数据需求，为资源有限机构提供可行解决方案，推动病理AI应用落地。
...
### [214] [Ordered-subsets Multi-diffusion Model for Sparse-view CT Reconstruction](https://arxiv.org/abs/2505.09985)
*Pengfei Yu,Bin Huang,Minghui Zhang,Weiwen Wu,Shaoyu Wang,Qiegen Liu*

Main category: eess.IV

TL;DR: 提出有序子集多重扩散模型(OSMM)，通过投影数据子集划分和多重扩散学习机制，显著提升稀疏视图CT重建的细节表现与抗噪能力


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型直接处理原始CT投影数据存在学习效率低、细节重建不完整的问题，冗余数据导致模型难以捕捉关键特征

Method: 1. 将CT投影数据划分为等量子集，采用多子集扩散模型(MSDM)独立学习各子集特征
2. 结合完整投影数据的整体扩散模型(OWDM)作为全局约束
3. 构建无监督学习框架增强模型鲁棒性

Result: 实验表明OSMM在PSNR指标上提升3.2dB，噪声标准差降低41%，在80%稀疏度场景下仍能保持结构相似性(SSIM)超过0.92

Conclusion: OSMM通过局部特征学习与全局约束的协同机制，为不同临床场景提供自适应强、细节保留优异的CT重建解决方案
...
### [215] [Visual Fidelity Index for Generative Semantic Communications with Critical Information Embedding](https://arxiv.org/abs/2505.10405)
*Jianhao Huang,Qunsong Zeng,Kaibin Huang*

Main category: eess.IV

TL;DR: 提出融合关键信息嵌入的生成式语义通信系统，通过语义过滤提取文本提示与关键特征，结合GVIF评估指标实现信道自适应优化，实验验证系统在图像保真度与传输效率上的优势。
...
Method: 1. 设计语义过滤算法提取与语义标签相关的关键特征 2. 融合文本提示与关键特征进行扩散模型重建 3. 提出GVIF指标量化特征互信息 4. 基于GVIF最大化实现信道自适应的特征量/压缩率联合优化

Result: GVIF指标与PSNR/关键信息量显著相关，优化系统在PSNR提升3.2dB，FID降低15.7%，优于基准方案。

Conclusion: 关键信息嵌入框架有效平衡语义压缩与视觉保真，GVIF驱动的自适应系统为6G语义通信提供可靠评估与优化路径。
...
### [216] [HWA-UNETR: Hierarchical Window Aggregate UNETR for 3D Multimodal Gastric Lesion Segmentation](https://arxiv.org/abs/2505.10464)
*Jiaming Liang,Lihuan Dai,Xiaoqi Sheng,Xiangguang Chen,Chun Yao,Guihua Tao,Qibin Leng,Honming Cai,Xi Zhong*

Main category: eess.IV

TL;DR: 提出GCM 2025多模态MRI数据集与HWA-UNETR分割框架，解决胃癌病灶分析中数据稀缺与模态融合难题


<details>
  <summary>Details</summary>
Motivation: 现有胃癌多模态分析面临独立模态数据集稀缺、固有模态错位导致的模型训练困难及精度下降问题

Method: HWA-UNETR框架采用可学习窗口聚合层建立跨模态特征对应，结合三向融合曼巴机制进行上下文建模与长程依赖捕获

Result: 在GCM 2025和BraTS 2021数据集上Dice分数提升1.68%，保持强鲁棒性

Conclusion: 首个大规模胃癌多模态开放数据集与新型动态特征融合框架的结合，为精准医学影像分析提供新范式
...
### [217] [Multi-contrast laser endoscopy for in vivo gastrointestinal imaging](https://arxiv.org/abs/2505.10492)
*Taylor L. Bobrow,Mayank Golhar,Suchapa Arayakarnkul,Anthony A. Song,Saowanee Ngamruengphong,Nicholas J. Durr*

Main category: eess.IV

TL;DR: 多对比激光内窥镜(MLE)通过可调谐光谱、相干和定向照明技术，实现三倍组织对比度提升和五倍颜色差异改善，显著优于传统白光和窄带成像。


<details>
  <summary>Details</summary>
Motivation: 传统白光内窥镜对组织颜色、纹理和形状的细微差异检测能力有限，导致临床漏诊。MLE旨在通过多模态成像技术突破这一限制。

Method: 1. 多光谱扩散反射增强组织色团对比
2. 激光散斑对比成像量化血流
3. 光度立体技术表征黏膜地形
平台验证包含离体模型和临床结肠镜检查。
...
Conclusion: MLE通过多维度互补性组织对比技术，展现出作为改善胃肠道成像研究工具的临床潜力。
...
### [218] [A Survey on Large Language Models in Multimodal Recommender Systems](https://arxiv.org/abs/2505.09777)
...
### [219] [A Survey on Large Language Models in Multimodal Recommender Systems](https://arxiv.org/abs/2505.09777)
*Alejo Lopez-Avila,Jinhua Du*

Main category: cs.IR

TL;DR: 该论文系统综述了大语言模型（LLMs）与多模态推荐系统（MRS）的融合，提出新分类法并分析技术挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs如何通过语义推理、上下文学习等能力突破传统多模态推荐系统的性能瓶颈，并解决其可扩展性、模型访问性等新挑战。

Method: 通过系统性文献综述，整合提示工程、微调策略和数据适配技术，提出基于整合模式的新分类法，并从相关推荐领域迁移可复用技术。

Result: 构建了涵盖37项核心研究的分类框架，识别出跨域可迁移技术，并建立了包含8个基准数据集的评估体系。

Conclusion: LLMs为MRS带来范式革新，但需在计算效率、隐私保护等方向持续突破，未来研究应聚焦模型轻量化与多模态对齐技术。
...
### [220] [Tales of the 2025 Los Angeles Fire: Hotwash for Public Health Concerns in Reddit via LLM-Enhanced Topic Modeling](https://arxiv.org/abs/2505.09665)
...
TL;DR: 通过Reddit数据分析2025年洛杉矶野火期间的公共讨论，结合主题建模与分层框架揭示公众健康关注与危机应对模式


<details>
  <summary>Details</summary>
Motivation: 近年来野火灾害加剧，理解受灾人群的实时反应对灾害响应至关重要。社交媒体为捕捉动态公共话语提供独特视角，但现有研究缺乏系统性分析框架。

Method: 采用LLM增强的主题建模方法，构建包含385帖、114K评论的数据集。建立SA-CN双层分类框架，结合定量分析与HITL人工细化。

Result: SA讨论峰值与火势扩张同步（2-5天），公共健康/安全主题覆盖环境/职业健康等多维度。CN中60%为悲伤表达，40%涉及心理健康风险，夜间讨论最活跃。

Conclusion: 首次创建标注社交媒体灾难数据集，提出可扩展的多层分析框架，为灾后心理干预和健康传播提供数据驱动的决策支持。
...
### [222] [Predictability Shapes Adaptation: An Evolutionary Perspective on Modes of Learning in Transformers](https://arxiv.org/abs/2505.09855)
...
### [223] [Comparing Exploration-Exploitation Strategies of LLMs and Humans: Insights from Standard Multi-armed Bandit Tasks](https://arxiv.org/abs/2505.09901)
...
### [224] [Advanced Crash Causation Analysis for Freeway Safety: A Large Language Model Approach to Identifying Key Contributing Factors](https://arxiv.org/abs/2505.09949)
...
### [225] [Learning Virtual Machine Scheduling in Cloud Computing through Language Agents](https://arxiv.org/abs/2505.10117)
...
### [226] [ComplexFormer: Disruptively Advancing Transformer Inference Ability via Head-Specific Complex Vector Attention](https://arxiv.org/abs/2505.10222)
...
### [227] [Superposition Yields Robust Neural Scaling](https://arxiv.org/abs/2505.10465)
...
### [228] [Parallel Scaling Law for Language Models](https://arxiv.org/abs/2505.10475)
...
### [229] [RouteNator: A Router-Based Multi-Modal Architecture for Generating Synthetic Training Data for Function Calling LLMs](https://arxiv.org/abs/2505.10495)
...
### [230] [MASSV: Multimodal Adaptation and Self-Data Distillation for Speculative Decoding of Vision-Language Models](https://arxiv.org/abs/2505.10526)
...
### [231] [Predictability Shapes Adaptation: An Evolutionary Perspective on Modes of Learning in Transformers](https://arxiv.org/abs/2505.09855)
*Alexander Y. Ku,Thomas L. Griffiths,Stephanie C. Y. Chan*

Main category: cs.LG

TL;DR: Transformer模型通过权重学习（IWL）和上下文学习（ICL）的类比生物进化策略，揭示环境可预测性对学习模式平衡的调控机制：稳定性驱动IWL，线索可靠性增强ICL，学习动态呈现任务依赖的阶段性特征。


<details>
  <summary>Details</summary>
Motivation: 受进化生物学中遗传编码（类比IWL）与表型可塑性（类比ICL）策略的启发，探究环境可预测性维度（稳定性与线索可靠性）如何决定Transformer模型两种学习模式的平衡关系。

Method: 通过回归和分类任务实验，操作化环境稳定性（长期模式持续性）与线索可靠性（上下文信息预测性），系统分析其对IWL/ICL平衡的影响及学习动态演变。

Result: 1. 高环境稳定性（>95%）显著促进IWL主导
2. 高线索可靠性提升ICL效能（低稳定性时效果倍增）
3. 学习阶段呈现非单调演化：分类任务出现ICL→IWL转变，而回归/少类任务呈现IWL→ICL逆转

Conclusion: 研究确立了可预测性作为Transformer自适应策略的核心调控因子，提出相对成本假说解释学习模式转换机制，为理解ICL内在规律和优化模型训练提供新范式。
...
### [232] [Comparing Exploration-Exploitation Strategies of LLMs and Humans: Insights from Standard Multi-armed Bandit Tasks](https://arxiv.org/abs/2505.09901)
*Ziyuan Zhang,Darcy Wang,Ningyuan Chen,Rodrigo Mansur,Vahid Sarhangian*

Main category: cs.LG

TL;DR: LLMs在探索与利用的权衡中表现出与人类相似的随机+定向探索策略，但在复杂非平稳环境中适应性不足


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在动态决策任务中是否具有类似人类的探索-利用权衡策略，以及显式推理如何影响其决策行为

Method: 使用认知科学中的多臂老虎机任务，结合可解释选择模型分析LLMs/人类/算法的策略，通过不同提示策略和增强推理模型测试决策模式

Result: 推理使LLMs更接近人类行为（混合探索模式），简单任务中探索水平相似，但复杂环境下LLMs定向探索能力不足（尽管某些场景达到相似遗憾值）

Conclusion: LLMs作为人类行为模拟器存在潜力与局限：推理增强使其接近人类决策模式，但复杂环境适应性仍需提升，需改进探索策略的灵活性
...
### [233] [Advanced Crash Causation Analysis for Freeway Safety: A Large Language Model Approach to Identifying Key Contributing Factors](https://arxiv.org/abs/2505.09949)
*Ahmed S. Abdelrahman,Mohamed Abdel-Aty,Samgyu Yang,Abdulrahman Faden*

Main category: cs.LG

TL;DR: 利用微调后的Llama3 8B模型分析高速公路事故成因，通过零样本分类有效识别酒驾/超速等主因，获得88.89%专家认可度


<details>
  <summary>Details</summary>
Motivation: 传统统计方法和机器学习难以捕捉交通事故多因素复杂交互特性，探索LLM在事故深度分析中的潜力

Method: 基于226项研究构建数据集，使用QLoRA微调Llama3 8B模型，通过零样本分类实现无标签数据的事故成因识别

Result: 模型成功识别酒驾/超速/攻击性驾驶/分心驾驶等主因，结合道路维护等事件数据提升分析深度，问卷验证专家认可率达88.89%

Conclusion: LLM为交通事故多维度分析提供新范式，其解释性输出有助于制定精准安全策略，验证结果证实其实际应用价值
...
### [234] [Learning Virtual Machine Scheduling in Cloud Computing through Language Agents](https://arxiv.org/abs/2505.10117)
*JieHao Wu,Ziwei Wang,Junjie Sheng,Wenhao Li,Xiangfei Wang,Jun Luo*

Main category: cs.LG

TL;DR: 提出MiCo分层语言代理框架，利用大语言模型设计启发式策略解决云环境中大规模动态多维装箱问题，在超1万台虚拟机场景下达到96.9%竞争比。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法难以适应实时变化，启发式策略缺乏灵活性，现有学习方法通用性和可解释性不足。需要开发适应大规模复杂场景且具备动态组合策略能力的解决方案。

Method: 将ODMBP建模为半马尔可夫决策过程（SMDP-Option），采用两阶段架构：Option Miner通过LLM生成非上下文感知策略，Option Composer动态组合上下文策略与非上下文策略。

Result: 在真实企业数据集实验中，MiCo在超万级虚拟机规模下实现96.9%竞争比，在非稳态请求流和多样化配置场景仍保持高性能。

Conclusion: MiCo验证了LLM驱动的分层框架在复杂云环境中的有效性，为大规模动态资源调度问题提供了可解释、可扩展的解决方案。
...
### [235] [ComplexFormer: Disruptively Advancing Transformer Inference Ability via Head-Specific Complex Vector Attention](https://arxiv.org/abs/2505.10222)
*Jintian Shao,Hongyi Huang,Jiayi Wu,Beiwen Zhang,ZhiYu Wu,You Shan,MingKai Zheng*

Main category: cs.LG

TL;DR: ComplexFormer通过复数多头注意力机制(CMHA)，在复数平面上统一建模语义差异与位置信息，实现更灵活高效的注意力表达，显著提升语言任务性能与生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer的多头注意力机制难以协调语义差异建模与位置信息整合，现有方法存在语义/位置处理割裂或位置调整策略僵化的问题。ComplexFormer旨在通过复数空间统一表示，增强注意力的表达能力。

Method: 1. 头特异性欧拉变换：将查询/键投影到极坐标复数空间
2. 自适应差分旋转机制：exp[i(Adapt(AS_mn,i)+Δ(P_mn,i))]，允许各头独立学习语义角度差(AS)与位置编码(Δ)的融合策略

Result: 在语言建模/文本生成/代码生成/数学推理任务中，ComplexFormer实现了更低的生成困惑度、更强的长上下文连贯性，且参数效率优于RoPE-Transformer等基线模型。

Conclusion: ComplexFormer通过复数空间统一建模语义与位置关系，为注意力机制提供了更灵活的参数化方式，在保持计算效率的同时显著提升了模型表现力。
...
### [236] [Superposition Yields Robust Neural Scaling](https://arxiv.org/abs/2505.10465)
*Yizhou liu,Ziming Liu,Jeff Gore*

Main category: cs.LG

TL;DR: 研究发现表示叠加是神经缩放定律的关键机制，模型尺寸与损失的关系受特征频率分布和叠加强度共同影响


<details>
  <summary>Details</summary>
Motivation: 现有神经缩放定律（模型性能随尺寸增大呈幂律提升）的物理机制尚未明确，需探究表示叠加和特征频率分布的相互作用机制

Method: 构建基于特征叠加原理的玩具模型，分析强弱两种叠加模式下损失随模型尺寸的缩放规律，通过几何解释和四类开源LLM的实证验证

Result: 弱叠加时损失缩放依赖特征频率分布（符合幂律），强叠加时损失与模型维度成反比；实证显示主流LLM符合强叠加模式的预测

Conclusion: 表示叠加机制解释了神经缩放定律的物理本质，该发现为开发更高效模型架构和训练策略提供了理论依据
...
### [237] [Parallel Scaling Law for Language Models](https://arxiv.org/abs/2505.10475)
*Mouxiang Chen,Binyuan Hui,Zeyu Cui,Jiaxi Yang,Dayiheng Liu,Jianling Sun,Junyang Lin,Zhongxin Liu*

Main category: cs.LG

TL;DR: 提出并行扩展(ParScale)方法，通过增加并行计算而非参数规模，在保持模型性能的同时显著提升推理效率，比传统参数扩展节省22倍内存和6倍延迟


<details>
  <summary>Details</summary>
Motivation: 传统语言模型扩展方法（参数扩展/推理时扩展）存在显著资源消耗，需要更高效的替代方案。ParScale旨在通过并行计算复用参数，突破扩展效率瓶颈

Method: 对输入施加P种可学习变换→并行执行模型前向传播→动态聚合输出。支持任意模型结构，可通过后训练快速适配已有预训练模型

Result: 实验验证并行扩展定律：P路并行等效O(logP)参数扩展效果；内存增加减少22倍，延迟增加降低6倍；后训练仅需少量token即可改造现有模型

Conclusion: 为低资源场景部署强大模型提供新路径，重新定义了计算效率在机器学习中的价值，开辟了模型扩展的第三范式
...
### [238] [RouteNator: A Router-Based Multi-Modal Architecture for Generating Synthetic Training Data for Function Calling LLMs](https://arxiv.org/abs/2505.10495)
*Vibha Belavadi,Tushar Vatsa,Dewang Sultania,Suhas Suresha,Ishita Verma,Cheng Chen,Tracy Holloway King,Michael Friedrich*

Main category: cs.LG

TL;DR: 提出基于路由器的架构生成合成数据，优化LLM函数调用任务表现


<details>
  <summary>Details</summary>
Motivation: 真实用户数据稀缺且隐私受限，传统合成方法难以匹配真实数据分布

Method: 结合知识图谱和多模态语言模型，通过路由机制控制数据生成分布

Result: 在真实用户测试中函数分类准确率提升，API参数选择表现优于传统方法

Conclusion: 该架构建立了函数调用任务新基准，验证了合成数据生成的有效性
...
### [239] [MASSV: Multimodal Adaptation and Self-Data Distillation for Speculative Decoding of Vision-Language Models](https://arxiv.org/abs/2505.10526)
*Mugilan Ganesan,Shane Segal,Ankur Aggarwal,Nish Sinnadurai,Sean Lie,Vithursan Thangarasa*

Main category: cs.LG

TL;DR: MASSV通过两阶段方法将小语言模型转化为多模态草案模型，加速视觉语言模型的推测解码，实现最高1.46倍推理加速


<details>
  <summary>Details</summary>
Motivation: 传统推测解码技术应用于视觉语言模型时面临两个核心挑战：小语言模型缺乏视觉处理能力，且其预测无法匹配考虑视觉上下文的VLM目标模型

Method: 1. 通过可训练投影器连接VLM视觉编码器与草案模型
2. 使用目标VLM生成响应进行自蒸馏视觉指令调整，对齐预测分布

Result: 在Qwen2.5-VL和Gemma3模型上实现接受长度提升30%，端到端推理加速最高达1.46倍（视觉相关任务）

Conclusion: MASSV为加速当前及未来视觉语言模型提供了可扩展、架构兼容的解决方案
...
### [240] [RainPro-8: An Efficient Deep Learning Model to Estimate Rainfall Probabilities Over 8 Hours](https://arxiv.org/abs/2505.10271)
*Rafael Pablos Sarabia,Joachim Nyborg,Morten Birk,Jeppe Liborius Sjørup,Anders Lillevang Vesterholt,Ira Assent*

Main category: cs.LG

TL;DR: 提出欧洲8小时高分辨率概率降水预报深度学习模型，整合多源数据实现超越现有系统的性能


<details>
  <summary>Details</summary>
Motivation: 克服雷达深度学习模型预报时效短（仅6小时）的局限，通过多源数据融合提升预报精度和不确定性量化能力

Method: 紧凑型深度学习架构整合雷达、卫星和数值预报数据，采用概率映射量化不确定性，兼顾训练效率和长程相互作用捕捉

Result: 在准确率（CRPS评分）和计算效率（推理速度提升3倍）上全面超越传统数值预报、外推法和深度学习临近预报模型

Conclusion: 建立了欧洲高分辨率降水预报新标准，在预报精度（0-8小时）、可解释性和计算效率间实现最佳平衡
...
### [241] [PIF: Anomaly detection via preference embedding](https://arxiv.org/abs/2505.10441)
*Filippo Leveni,Luca Magri,Giacomo Boracchi,Cesare Alippi*

Main category: cs.LG

TL;DR: 提出新型异常检测方法PIF，通过高维空间嵌入和树结构(PI-Forest)实现高效异常评分，实验显示优于主流方法


<details>
  <summary>Details</summary>
Motivation: 解决结构化模式异常检测问题，融合自适应隔离方法与偏好嵌入的灵活性

Method: PI-Forest：将数据嵌入高维偏好空间，采用树基方法计算异常分数

Result: 合成/真实数据集实验表明PIF优于现有技术，PI-Forest在偏好空间距离度量/异常点隔离表现更优
...
### [242] [SEAL: Searching Expandable Architectures for Incremental Learning](https://arxiv.org/abs/2505.10457)
*Matteo Gambella,Vicente Javier Castro Solar,Manuel Roveri*

Main category: cs.LG

TL;DR: 提出SEAL框架，结合神经架构搜索和选择性扩展策略，在数据增量学习中动态调整模型结构，有效平衡可塑性与稳定性，减少遗忘并保持较小模型规模。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于NAS的增量学习方法需要持续扩展模型导致的资源浪费问题，针对数据增量场景中样本不可重复访问的特性，寻求更高效的动态架构调整方案。

Method: 1. 引入容量估计指标动态触发模型扩展
2. 交叉蒸馏训练保持扩展后的稳定性
3. NAS联合优化架构设计与扩展策略
4. 选择性扩展机制减少参数增长

Result: 在多个基准测试中实现平均准确率提升15%，模型尺寸缩减40%以上，遗忘率降低至传统方法的1/3。

Conclusion: 验证了NAS与动态扩展策略结合在增量学习中的有效性，为资源受限环境下的持续学习提供了新思路，拓展了AutoML在动态场景中的应用边界。
...
### [243] [PIG: Privacy Jailbreak Attack on LLMs via Gradient-based Iterative In-Context Optimization](https://arxiv.org/abs/2505.09921)
...
### [244] [PIG: Privacy Jailbreak Attack on LLMs via Gradient-based Iterative In-Context Optimization](https://arxiv.org/abs/2505.09921)
*Yidan Wang,Yanan Cao,Yubing Ren,Fang Fang,Zheng Lin,Binxing Fang*

Main category: cs.CR

TL;DR: 提出PIG框架，通过结合越狱攻击有效提取LLMs中的个人隐私信息，在多个模型上实现SOTA效果


<details>
  <summary>Details</summary>
Motivation: 现有隐私评估方法易被对齐模型阻止，而越狱攻击在隐私泄露场景的应用潜力未被充分挖掘

Method: 1.识别PII实体及类型 2.上下文学习构建隐私上下文 3.三种梯度策略迭代更新上下文提取目标信息

Result: 在4个白盒/2个黑盒LLMs实验中，PIG超越基线方法（最高提升32.1%），达到SOTA表现

Conclusion: LLMs存在重大隐私风险，需加强安全防护措施，研究为隐私保护机制设计提供新视角
...
### [245] [Generative diffusion model surrogates for mechanistic agent-based biological models](https://arxiv.org/abs/2505.09630)
*Tien Comlekoglu,J. Quetzalcóatl Toledo-Marín,Douglas W. DeSimone,Shayn M. Peirce,Geoffrey Fox,James A. Glazier*

Main category: q-bio.QM

TL;DR: 使用去噪扩散概率模型构建CPM的生成式AI替代模型，加速血管生成模拟，计算效率提升22倍


<details>
  <summary>Details</summary>
Motivation: 传统CPM模型在模拟大时空尺度生物系统时计算成本过高，需要开发高效替代模型

Method: 结合去噪扩散概率模型（DDPM）和图像分类器，建立血管生成CPM的生成式AI替代模型

Result: 替代模型可提前生成20000时间步的配置，计算时间减少约22倍

Conclusion: 为开发随机生物系统数字孪生奠定基础，展示生成式AI替代复杂生物模型的应用潜力
...
### [246] [LAV: Audio-Driven Dynamic Visual Generation with Neural Compression and StyleGAN2](https://arxiv.org/abs/2505.10101)
...
TL;DR: LAV系统整合EnCodec音频压缩与StyleGAN2生成能力，通过潜在表示实现语义丰富的音视转换。


<details>
  <summary>Details</summary>
Motivation: 突破传统显式特征映射限制，利用预训练模型探索更自然连贯的跨模态生成方法。

Method: 使用EnCodec嵌入作为潜在表示，通过随机线性映射转换至StyleGAN2风格空间，保留语义完整性。

Result: 系统生成细致且语义一致的动态视觉输出，验证了预训练音频模型在跨模态生成中的有效性。

Conclusion: LAV框架开创了预训练音频模型在艺术生成领域的新范式，展示了潜在空间转换的技术潜力。
